...
3. **先问能否从结构上消除**：准备新增守卫、代际计数器、会话快照比对、busy 标志或补救注释前，先按 `.agents/engineering-invariants.md` §0 检查已有结构化机制能否直接覆盖，能覆盖就复用。同一类手写守卫已在两处以上出现、本次要写第三处时，优先抽取或扩展共享机制；语义确实不同或改动面超出本次任务时，在最终回复中说明理由并列为结构性遗留，不要默默复制。
