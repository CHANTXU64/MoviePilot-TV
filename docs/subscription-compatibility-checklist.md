...
- `MoviePilot-TV/Services/APIService.swift`
  - `/subscribe/`、`/subscribe/media/{mediaid}` 等订阅接口必须至少受 `canAccess(.subscribe)` 控制；媒体服务器配置、系统设置等状态或配置接口必须按 `canRequestSuperUserEndpoints` 或后端真实权限单独控制。
  - `checkSeasonsNotExists(mediaInfo:)` 只能作为可选的媒体服务器状态查询；当前后端和 Web 允许普通订阅用户读取 `/mediaserver/notexists`，TV 端应以 `canAccess(.subscribe)` 作为入口 guard，并保证权限失败时返回空状态、保留订阅主流程、不能触发会话登出。
