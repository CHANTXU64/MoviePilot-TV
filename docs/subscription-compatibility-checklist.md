...
- `MoviePilot-TV/Services/APIService.swift`
  ...
  - Dashboard、下载管理、首页最近入库、整理历史等 Web 菜单入口按 `permissions.manage` 对齐；不要用 `canRequestSuperUserEndpoints` 或 `Token.super_user` 挡 `fetchStatistic` / `fetchStorage` / `fetchDownloaderInfo` / `fetchDownloading` / `startDownload` / `stopDownload` / `deleteDownload` / `fetchMediaServers` / `fetchMediaServerLatest` / `fetchTransferHistory` / `fetchStorages` / `deleteTransferHistory` / `aiRedoTransferHistory` / `manualTransfer`。即使后端最终对部分动作做超管校验，TV 端也不在 `APIService` 或 ViewModel 里伪造成功/失败，真实错误交给后端返回。
  - 搜索过滤用到的只读配置（例如 `CustomFilterRules`、`UserFilterRuleGroups`）按对应搜索/订阅入口控制；后端 `GET /system/setting/{key}` 当前是登录用户可读，不要把读取和本地应用过滤规则误收窄成 `Token.super_user`。写入 `POST /system/setting/{key}` 由后端最终鉴权，TV 端不要为了兜底批量吞错。
