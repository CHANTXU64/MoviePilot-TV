...
class StatusViewModel: ObservableObject {
  ...
  func stopDownload(hash: String) async -> Bool {
    ...
    do {
      let (success, message) = try await apiService.stopDownload(clientName: selectedClient, hash: hash)
      if !success {
  ...
  func startDownload(hash: String) async -> Bool {
    guard !selectedClient.isEmpty else { return false }
    do {
      let (success, message) = try await apiService.startDownload(clientName: selectedClient, hash: hash)
      if !success {
  ...
  func deleteDownload(hash: String) async {
    guard !selectedClient.isEmpty else { return }
    do {
      let (success, message) = try await apiService.deleteDownload(clientName: selectedClient, hash: hash)
      if success {
        // 仿照 Vue 的行为，从本地数组中移除项目以立即更新 UI。
