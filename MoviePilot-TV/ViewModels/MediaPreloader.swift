...
enum MediaDetailBackgroundImage {
  ...
  nonisolated static func cacheSecondPageImage(
    ...
    guard !cache.isCached(forKey: url.cacheKey, processorIdentifier: processor.identifier)
    else {
      return true
    }
    ...
    cache.store(image, forKey: url.cacheKey, processorIdentifier: processor.identifier)
    return true
  }
}
...
class MediaPreloadTask: ObservableObject {
  ...
  private func retrieveHeroImage(
    ...
    let options = MediaDetailBackgroundImage.heroOptions(
