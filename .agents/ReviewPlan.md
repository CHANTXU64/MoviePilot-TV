...
| V001 | 已闭环 | 主审与独立复核已闭环；G08第三裁曾将F-107升P1，后续`90b40b4`修复主触发，剩余晚到show降P2且用户决定跳过；F-108未验证、用户决定跳过修复（保留运行边界）；H-012已确认，F-049升级P2，F-091/F-093仍须按错误episode避免轮询重复常驻通知 |
...
| V002-A | 已闭环 | 主审/独立/G06闭合；F-109/F-111均P2，四类profile key、credential/currentUser身份分裂与迁移边界已登记；F-109 已修复（`90b40b4` 统一会话权威时四类 key 迁移到 `profileKey`（baseURL\|user:user_id）并一次性迁移旧键，`_` 分隔碰撞对不再成立；旧碰撞数据无法无损还原，按原结论接受） |
...
| V004-A | 已闭环 | 主审与独立复核已闭环；I005补强后F-115升P2、F-117维持P3；后续G03纠偏以两张正确映射票将F-116升级确认P2，实际闪烁时长/焦点仍待运行；后续内存优化复核确认F-117仅缓解晚到内存结果、handle安装竞态仍在，用户决定跳过（2026-09-17）；F-100传播闭合 |
| V004-B | 已闭环 | 主审与独立复核已闭环；F-119最终P2，后续内存优化复核确认一对多任务与first-match回写仍在，但配套Web v3.0.1同样可能让已挂载的同媒体实例保留旧订阅标签，用户决定跳过TV单端增强（2026-09-17）；后续G03窄第三裁确认F-118 ownerless pin根因P2，push/Tab/State端到端时序仍待运行 |
...
| V009-E | 已闭环 | 主审与独立复核已闭环；F-129/F-131/F-132 维持确认，F-134 无部署复合 fixture、F-136 无 TV 产品意图、用户决定跳过修复，保留未验证边界；F-088 扩展维持 |
| V009-F | 已闭环 | 主审与独立复核已闭环；无新编号，F-129/F-130 与 F-132 共有值扩展确认，传播会话/分页/分享/响应/query 发现；F-133/F-134 已按官方 IMDb fixture 修复，F-136 未验证且用户决定跳过修复，F-135 已确认 P3 |
| V010 | 已闭环 | 主审与独立复核已闭环；后续G01/G04双票将F-138升条件P1、F-139升P2并已修复（2026-08-21：推荐页由ContentView传入Tab选中状态，`.task(id: isSelected)`在false→true边沿调用现有成功空恢复；详情/合集保留既有再激活恢复）；推荐分页/session/身份/图片/动作传播不变 |
...
| V022-C | 已闭环 | 分段双审、I009与G09复核完成；F-154经G09两票驳回并归F-232稳定排序测试，F-155/F-232维持P2；F-204历史裁决维持条件P1，TV修复已由`81d42fb`提交；F-155 已修复（2026-08-18） |
| V022-D | 已闭环 | 分段双审、I009与G09复核完成；F-098/F-156升条件P1，F-070升P2；F-080/F-075/F-203等terminal/session/partial outcome传播闭合；F-156 已修复（2026-08-18） |
| V023 | 已闭环 | 主审、独立与G06完成；F-157按不可恢复错误终态升P2，失败/取消占用检查key后同key成功被guard吞且前台固定不判定；其余会话传播闭合；F-157 已修复（2026-08-21：失败不占terminal key，前台成功收敛仍存在的警告且不重建用户已关闭的同key低版本警告） |
| C001 | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；后续G05把P2稳定后果收窄锚定到DownloadTask主行空Button，F-158升P2；其余透明sink真实落焦/VoiceOver频率仍留运行验证；F-158 用户跳过（2026-08-20） |
| C002 | 已闭环 | review_a001_h 主审、review_a001_j 独立复核完成；F-159确认P3：5文件6处生产show均无主动announcement，同文案事件不能靠值变化监听；F-107独立、F-108未验证且用户决定跳过修复；F-159 用户跳过（2026-08-20） |
| C003 | 已闭环 | 双审、G10与G09复核完成；F-160确认P2，F-161按静态透明Button/focus绑定升条件P2，实际Focus Engine/VoiceOver频率仍未验证；F-156传播随G09升条件P1；F-160 用户跳过（2026-08-20） |
| C004 | 已闭环 | 双审与G10/G09复核完成；F-162/F-165升P2，F-163未验证且用户决定跳过修复，F-164 已修复（2026-09-15）；F-165只陈述内容内退出/辅助可发现性，不声称无法退出；F-162 已修复（2026-08-20） |
| C005 | 已闭环 | verify_a001_h 主审、review_a001_h 独立复核及verify_a001_h补充裁决完成；F-166驳回，F-167未验证且用户决定跳过修复：托管根transform契约违反确定、可见故障未验证 |
| C006 | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；后续G05按丢title/结构化selected语义将F-168升P2；季100初始焦点与VoiceOver/checkmark播报仍未验证；F-168 用户跳过（2026-08-20） |
...
| C012 | 已闭环 | review_a001_h 主审、review_a001_j 独立复核完成；F-178条件性P3，用户决定跳过修复：评分备用名与卡片展示名分裂；F-076/F-172/F-174/F-177传播闭合，固定高度仅留运行盲点；F-174 用户跳过（2026-08-20） |
...
| C017 | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；后续G05将资源卡/筛选空白字符串分裂F-179升条件P2；促销badge仅留契约盲点，既有资源/解析/控制语义传播闭合 |；F-179 已修复（2026-08-20）
| C018-A | 已闭环 | review_a001_j 主审、review_a001_h 独立复核完成；无新编号，F-022/F-032/F-057/F-058/F-059/F-061/F-110/F-158传播闭合，onAppear首帧与同ID变化仅留盲点；F-158 用户跳过（2026-08-20） |
...
| C018-C | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；无新编号，F-110/F-057…F-059/F-179/F-168/F-170传播闭合，F-061根在A段，F-163用户决定跳过修复、F-164 已修复（2026-09-15）、F-165有反证；F-165 用户降级P3（2026-08-20） |；F-179 已修复（2026-08-20）
| W001 | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；无新编号，F-076/F-099/F-172/F-158/F-165传播闭合，F-178已确认且用户决定跳过修复，F-177已修复但真机性能/像质仍未验收，AddDownload media_in分工仅留契约边界；F-158 用户跳过（2026-08-20） |
| W002 | 已闭环 | review_a001_j 主审、review_a001_h 独立复核及G06后裁完成；无新编号，F-086/F-088/F-107/F-027/F-062/F-063/F-159传播闭合，F-029本View无新增触发、F-089最终确认P2，no-access首次登录顺序通过；F-159 用户跳过（2026-08-20） |
| W003 | 已闭环 | review_a001_h 主审、review_a001_j 独立复核及遗漏项补裁决完成；无新编号，Home订阅/媒体库/TMDB资源/session通知/卡片既有传播闭合；后续G03确认F-118 P2，F-012/F-017边界不变，F-158不适用；当前全文件486行覆盖原范围，前三次失败输出均作废；F-158 用户跳过（2026-08-20） |
| W004 | 已闭环 | verify_a001_h 主审、review_a001_j 独立复核完成；无新编号，F-077/F-078/F-120/F-121/F-129…F-132/F-033/F-035/F-105/F-106/F-027/CHK-005传播闭合；F-133/F-134 当时未验证、后由官方 IMDb fixture 确认并修复，F-136未验证且用户决定跳过修复，F-135已确认P3，F-039/F-158不适用；F-158 用户跳过（2026-08-20） |
| W005 | 已闭环 | review_a001_j 主审、review_a001_h 独立复核完成；无新编号，推荐货架/空态、身份、分页恢复、生命周期session、订阅动作、卡片图片与可访问性既有finding传播闭合；F-079后经当前官方schema裁决确认P2，F-173未验证且用户决定跳过修复，Fork presenter/合集route维持未验证，F-158不适用；F-158 用户跳过（2026-08-20） |
| W006-A | 已闭环 | review_a001_h 主审、review_a001_j 独立复核完成；无新编号，F-130/CHK-005/F-142/F-039/F-076/F-114/F-121/F-027/F-137/F-140/F-141/F-112/F-168传播闭合，键盘提交按显式双模式按钮契约不立项，F-169不适用；F-168 用户跳过（2026-08-20） |
| W006-B | 已闭环 | review_a001_h 主审、review_a001_j 独立复核完成；无新编号，F-033/F-034/F-035/F-036/F-039/F-076/F-027/F-130/CHK-005/F-137/F-138/F-140/F-141/F-142/F-077/F-079/CHK-009/F-104/F-143传播闭合，F-178已确认且用户决定跳过修复，F-064维持未验证，F-139/F-158不适用；F-158 用户跳过（2026-08-20） |
| W006-C | 已闭环 | review_a001_j 主审、review_a001_h 独立复核完成；无新编号，分页会话、媒体/人物/分享身份、菜单订阅、人物展示导航、卡片可访问性与图片链既有finding传播闭合，F-079后经当前官方schema裁决确认P2；F-064维持未验证，F-177已修复但真机性能/像质仍未验收，F-173未验证且用户决定跳过修复，F-158/F-176不适用；F-158 用户跳过（2026-08-20） |
| W006-D | 已闭环 | review_a001_h 主审、review_a001_j 独立复核完成；无新编号，排名/身份/展示/类型图片、分享人物导航、会话预加载、sourceFrame与菜单既有finding传播闭合，F-064与双FocusState/VoiceOver/性能维持未验证，F-177已修复但真机性能/像质仍未验收，F-171/F-173/F-175不适用；F-171 用户跳过（2026-08-20）；F-175 用户跳过（2026-08-20） |
| W007 | 已闭环 | 分段三方与I013最终第三裁完成；F-180升条件性P2；后续G03两票以确定的cache-hit揭示顺序将F-116升级确认P2，实际闪烁时长/焦点仍待运行；其余传播闭合 |；F-180 已修复（2026-08-20，Logger+全局失败通知）
| W008-A | 已闭环 | 分段三方与I013最终第三裁完成；F-181保持未验证但条件后果校准为P2：Hero先false/Content后true静态缺口成立，真实tvOS事件顺序须Simulator/真机验证；既有状态/session/lifecycle传播闭合 |；F-181 用户跳过（2026-08-20）
| W008-B | 已闭环 | 分段双审及I008/I013回溯完成；F-182升P2，旧false/空分季无法发现远端新建订阅；F-007/F-047…F-049/F-130及Header/session传播闭合 |；F-182 用户跳过（2026-08-20）
| W008-C | 已闭环 | 分段三方及I013回溯完成；F-183已修复（2026-09-16）：详情页“TMDB详情页”按钮保留预识别期间的原有disabled门禁，动作入口同步防重入且不新增任务期间disabled；定向测试1/1、反向校验与clean build通过，未跑兼容测试；新增F-231确认P2承载TMDB异步动作离开route后晚到；Hero/投影/图片转场等传播闭合 |
...
| W009 | 已闭环 | review_a001_h主审、review_a001_j独立复核与verify_a001_h第三裁决完成；F-185确认P2：足够长合法biography在无ScrollView/分页/可移动焦点锚点的“完整简介”模态Sheet中必有不可达尾部。加载/无简介空action Button及空作品focusable Text并入F-158同根传播；人物详情失败并入F-126，F-143/F-144等传播闭合，F-176不适用；F-158 用户跳过（2026-08-20） |；F-185 用户跳过（2026-08-20）
...
| W011 | 已闭环 | 主审、独立复核与verify_a001_h第三裁决完成；后续G05将F-110升P2，F-186/F-187维持P2。筛选零项焦点归F-158（后续G05 P2）、missingSites归F-080、null因子解码归F-022，AddDownload生命周期/session归F-147/F-027，其余传播闭合；F-158 用户跳过（2026-08-20）；F-186 已修复（2026-08-20）；F-187 用户跳过（2026-08-20） |
...
| W013-C | 已闭环 | 主审、独立复核与review_a001_j第三裁决完成；长overview并入F-185，F-190已修复（2026-09-15）：季卡与详情Sheet共用SeasonDisplayFormatter，定向3/3、反向还原失败校验、恢复后3/3与Simulator clean build通过；F-191已修复（2026-09-15）：两个Sheet海报外层固定360×540，定向1/1与Simulator clean build通过。无写操作owner；真实四态海报形态、焦点/可访问性及两位复核代理既往源码暴露维持运行盲点 |；F-185 用户跳过（2026-08-20）
| W014 | 已闭环 | F-147 Subscribe子项已由`a872737`修复；F-148条件P1已后续处置，F-199已由`ce7afcc`修复，F-170/F-195/F-200 P2；F-069经当前合同复核不再是现行缺陷并转CHK-003，CHK-013/014已确认，advanced a11y未验证；F-170 已修复（2026-08-20）；F-195 用户跳过（2026-08-20） |
| W015 | 已闭环 | 双审与G06完成；F-193跨服同号ID的原条件P1链已由`90b40b4`闭合，当前仅剩同profile operation/presentation竞争P2；F-194 P2，F-191已修复（2026-09-15）：Fork海报外层固定360×540；F-027 P1/CHK-005及F-008/F-121/F-185传播闭合；F-164 已修复（2026-09-15），真实海报四态/焦点仍未单独验证；F-185 用户跳过（2026-08-20）；F-194 用户跳过（2026-08-20） |
| W016 | 已闭环 | 双审、第三裁与G09完成，W017下载子链既往暴露已披露；F-149升P1，F-150维持P2，F-198升P2；F-192按用户确认范围由`b304b58`完成Web同款展示过滤，后端对象级授权风险明确留存范围外；Transfer转W019；F-198 已修复（2026-08-21） |
...
| W018-A | 已闭环 | 双审与G09完成且既往外围暴露已披露；F-188/F-189旧v2.14.4快照裁决后经目标v2.15.1复核驳回（已含`3b709b7`）；F-156升条件P1，F-147整体为条件P1（本段只补P2子后果）；F-206维持P2，F-075/F-074/F-162/F-168确认、F-163未验证且用户决定跳过修复、F-164 已修复（2026-09-15）、F-166驳回；F-156 已修复（2026-08-18）；F-162 已修复（2026-08-20） |
| W018-B | 已闭环 | 双审与G09完成且既往源码/ledger暴露永久披露；clean-room窄裁将F-073收窄为data/item缺失fail-open并确认P2且已修复（`e8cdaf7`），F-151 P1且用户按当前官方Web v2对齐决定跳过，F-162/F-165 P2，其余intent/logID边界不变；F-162 已修复（2026-08-20） |
| I011 | 已闭环 | 集成及第三裁完成；F-061按默认稳定覆盖后端策略与软过滤承诺升P2，TorrentCard并入F-175且按下载主入口升P2；F-032/F-057…059/F-110/F-168/F-179/F-186传播闭合，同ID更新由I012/F-219裁决；F-168 用户跳过（2026-08-20）；F-175 用户跳过（2026-08-20） |；F-179 已修复（2026-08-20）；F-186 已修复（2026-08-20）
| I012 | 已闭环 | F-219驳回；F-076原跨owner P1链闭合、当前余项P2；F-103/F-036/F-035/F-039/F-137 P2；G04 clean-room窄裁全部闭合 |
| I005 | 已闭环 | review_a001_h完成MediaPreloader整文件集成，verify_a001_h定向独立复核；F-221 P2，F-220并入F-115后驳回且F-115升P2；后续I013/G02/G03裁决将F-180/F-118/F-119升P2、F-184升条件P1；F-117维持P3并在后续内存优化复核后由用户决定跳过，F-119在当前实现与配套Web复核后决定跳过TV单端增强（均为2026-09-17）；其余传播校准 |；F-180 已修复（2026-08-20，Logger+全局失败通知）
| I006 | 受限已闭环 | F-233/F-234/F-235/F-236 P2、F-237驳回；F-238复核确认配套Web v3.0.1同样保留`api_path`查询并追加筛选参数，标准FastAPI标量取后值，用户决定跳过TV单端处理（2026-09-17）；G04末裁补F-236当前合同，严格独立文件集成限制不变 |
...
| G03 | 已闭环 | 三票/窄第三裁完成；F-245独立P2，F-116/F-097/F-118/F-171/F-174/F-221均收敛P2，F-219驳回，F-173代码级重复处理确认但用户决定跳过修复，F-177代码已修复、真机运行未验收；CHK-006当前仅保留同季重复/超管fan-out范围不完整P1，非TMDB跨季子证据已由现行后端反证；CHK-017补Fork；既往源码暴露永久披露；F-171 用户跳过（2026-08-20） |
| G04 | 已闭环 | 既有错号意见作废；全新clean-room代理逐项确认F-034/F-035/F-039/F-137/F-142/F-143/F-176/F-236均P2。F-035/F-039共享owner取消实现但保留独立回归；未运行验证；F-176 已修复（2026-08-20） |
| G05 | 已闭环 | 主审错号CHK票作废；八项共同升级既已落账。G09进一步把F-102转未验证P3；F-133/F-134 当时继续未验证，后由官方 IMDb fixture 确认并修复；F-238后续经配套Web与FastAPI合同复核由用户决定跳过（2026-09-17），F-135维持P3；六项CHK保持当前精确边界，无新finding、无运行验证 |
| G06 | 已闭环 | F-019/F-062/F-063条件P1；F-193的历史跨profile P1链已由`90b40b4`闭合、余项P2；F-030/F-031/F-084/F-109/F-089 P2，F-157 已修复（2026-08-21）；后续G02末裁把F-087/F-121升P2，F-244并入F-130。既往源码暴露永久披露，未运行验证 |
...
| G08 | 已闭环 | 历史裁决F-107/F-148 P1；后续F-107主触发已由`90b40b4`修复，剩余晚到show降P2且用户决定跳过。F-049/F-121/F-223 P2，F-108未验证P3且用户决定跳过修复、F-159 P3；G02末裁覆盖F-121旧P3，H-012仅传播；既往通知源码污染永久披露 |
| G07 | 已闭环 | 三方裁决完成：F-143/F-036/F-227/F-226确认P2，F-228确认P3且用户决定跳过TV端修复（2026-09-16）；F-050合并F-056并维持P3，F-045/F-051/F-055维持P3，F-175维持P2；F-189旧v2.14.4链曾升条件P1，目标v2.15.1复核已驳回并转CHK-019升级边界；F-053当前无生产caller；内嵌人物窄化导演，Paginator另归F-034，最佳结果旧说法驳回，AniList插件/运行辅助功能保持未验证；三代理既往人物/Search源码污染永久披露；F-175 用户跳过（2026-08-20） |
| G10 | 已闭环 | review_a001_h完成37文件主审，verify_a001_h独立复核23文件/6,270行；F-229已修复（2026-09-16）：多选即时生效，按钮改为“完成”，focused test 1/1与clean build通过，未跑兼容测试；F-230确认P2，F-120/F-160/F-205升P2；Subscribe/session/Fork/manual-search/长文本分别归F-148/F-027/F-193/F-076/F-185，空Button/反馈/Picker归F-158/F-162/F-168，F-163用户决定跳过修复，F-164 已修复（2026-09-15）、F-165等传播/运行边界闭合；两代理既往业务Sheet源码污染永久披露；F-158 用户跳过（2026-08-20）；F-162 已修复（2026-08-20） |；F-185 用户跳过（2026-08-20）
...
| I008 | 已闭环 | review_a001_j完成MediaDetailViewModel全文件集成，review_a001_h定向独立复核；F-007升P1、F-182升P2；后续G03窄第三裁确认F-118 ownerless pin根因P2而保留端到端运行边界，F-047/F-048/F-049/F-130映射不变；两代理既往详情/订阅调用链污染永久披露 |；F-182 用户跳过（2026-08-20）
| I009 | 已闭环 | 整文件集成、定向复核与G09完成；F-098/F-156/F-204/F-203为P1，F-071/F-155/F-205/F-232为P2，F-153/F-154经两票驳回；F-204 TV修复已由`81d42fb`提交，F-027/F-072/F-033/F-080/F-075传播闭合，既往源码污染永久披露；F-155 已修复（2026-08-18）、F-156 已修复（2026-08-18） |
| I010 | 已闭环 | review_a001_j整文件集成、verify_a001_h独立复核及review_a001_h第三裁完成；F-020升条件性P1、F-239确认P2；后续G03将F-171/F-174升P2，poster仍留F-123，F-124/F-027/F-026/F-175/F-184传播不变；既往源码暴露永久披露；F-171 用户跳过（2026-08-20）；F-175 用户跳过（2026-08-20） |
| I013 | 已闭环 | verify_a001_h完成MediaDetailView 1113/1113行集成，review_a001_h定向复核，review_a001_j最终第三裁；F-231 P2、F-184条件P1、F-180 P2、F-181未验证条件P2、F-033根P2/详情局部P3；后续G03将F-116升确认P2；其余映射与既往源码暴露披露不变 |；F-180 已修复（2026-08-20，Logger+全局失败通知）；F-181 用户跳过（2026-08-20）
...
| I016 | 受限已闭环 | G01/G06完成等级裁决：F-106 P2、F-240 P2、F-089按当前后端401转确认P2；F-111/F-112 P2、F-242 P3且用户决定跳过（2026-09-16）、F-241已由`05ba3bd`修复，F-208为P3且用户决定跳过（2026-09-15），其余传播不变。三代理均曾主审W020分段，严格零暴露缺口永久披露 |
| W019 | 已闭环 | 双审、I009/G10/G09回溯完成；F-202 历史嵌套解码修复已完成（`670cf86`），验证及独立复审通过；F-152已由`fc0cefa`让弹窗文案/action共用完整对象快照并由ViewModel持有离页后继续的任务，完整门禁通过且补测后同一独立复审者PASS；F-203升P1后用户决定跳过本地后端修复，F-201/F-205维持P2，F-204历史裁决维持条件P1且TV修复已由`81d42fb`提交，F-232 P2；F-156当前态另行复核，F-153/F-154驳回，F-165/F-185传播闭合 |；F-185 用户跳过（2026-08-20）
...
| W020-B | 已闭环 | 双审完成；F-208确认P3，用户决定跳过TV端修复（2026-09-15）；F-130/CHK-005与F-185传播，F-161维持运行未验证；空白非法route确定，但Back/Menu投递与恢复/Focus结果不作静态保证，本段无手势滑动入口；披露W020-A既往边界及误显W020-C头20行未用于结论；F-161 用户跳过（2026-08-20） |；F-185 用户跳过（2026-08-20）
| W020-C | 已闭环 | 双审及F-216定向复核完成；F-207已修复（2026-09-15）：同账号同权限重登成功后`SystemViewModel.relogin()`复用`loadSystemInfo`刷新连接信息，定向回归1/1、反向失败校验、恢复后测试及tvOS Simulator clean build通过，兼容测试未跑且真实页面时序未验证。刷新401/403先logout后局部错误在新Login不可达的机制成立，但F-216重复编号并入扩展后的F-107根错误owner，状态码分类交叉F-089；旧登录/settings/身份/长文本等传播闭合 |
...
| W020-F | 已闭环 | 双审完成；无新编号，失权route/focus归F-130/CHK-005，加载/预览归F-126/F-085，初焦清除当前来源/规则归F-168。review_a001_h补充P2 Reduce Motion建议无新增静态后果且与既有票冲突，F-208维持P3；Back/VoiceOver交I016；F-168 用户跳过（2026-08-20） |
| W020-G | 已闭环 | 三代理完成；F-217确认独立P3，用户未遇到并决定跳过TV端修复（2026-09-15）：条件Exit modifier在pop离场窗口改变结构身份并重启推荐task，但根StateObject保留、只读GET与无稳定主路径中断不足P2；恒定modifier修复与通用task owner互不替代。window/Menu/Sheet仍交I016运行验证 |
...
| R001 | 已闭环 | 三代理确认F-218独立条件性P3，现已修复（2026-09-15）：已有会话首帧先进入准备状态，用户信息恢复完成后再构造认证Tab/Home；定向回归1/1、反向失败校验、恢复后测试通过，兼容测试未跑且真实认证帧/Home task待运行验证。F-106负责会话恢复后必要settings完成前撤门，F-130/CHK-005负责异步owner，三者不可互替。根MediaAction跨logout归F-130/CHK-005 |
| R002 | 已闭环 | 双审完成且review_a001_j披露R001时仅见第6/11行owner命中；无新编号，App级旧session通知跨logout/排队重排并入F-107，但须保留显式一次性logout原因交接；媒体handler归R001/F-130/CHK-005，Sheet层与VoiceOver归F-108/F-159运行验证，App注入/初始化通过；F-159 用户跳过（2026-08-20） |
...
| B006-A | 已闭环 | F-037 未验证 P3、用户决定跳过修复；F-038 已确认 P3；H-006 本段无违反；B006-C/S006/I002 后续复核均已闭环 |
| B005 | 已闭环 | F-040/F-041/F-044/F-045 已确认 P3；H-006 数据源层成立 |
| B006-B | 已闭环 | F-042 已修复 P3（仅 trim + 大写归一）；F-043 已确认 P3；共享空值展示边界但保持语言/国家标准分离 |
...
- `G09 / F-161/F-162/F-165`：透明原生Button须退出focus/accessibility树，长反馈须有完整读取入口，业务Sheet须有内容内关闭；全部复用原生控件/现有ScrollView，不建UI框架。；F-161 用户跳过（2026-08-20）；F-162 已修复（2026-08-20）
...
- `V001 / F-108`：未验证、条件性 P3；用户决定跳过修复，保留静态触发与错误清空链、tvOS Sheet 层级及五秒可见窗口未验证的边界；不再安排运行验收。回溯 C002/R001/V017/W013/V022/W019/G08。
...
- `V004-A / F-117`：已确认 P3，用户决定跳过（2026-09-17）；后续内存优化已增加请求前取消检查、`MPImageWarmer`取得handle后的取消、continuation exactly-once与晚到内存结果代际丢弃，但直接Kingfisher路径的取消标记和`activeImageDownload`安装仍未原子交接，且图片预取返回后仍缺发布`isDetailReady`前的取消复查。竞态窗口极小、无运行证据，当前主要影响为已取消请求继续消耗网络/解码资源及旧预载对象晚写ready；用户认为继续改动更易引入问题，明确保持现状；回溯 V004-B/V023/R001/R002/I005/G03/G06。
- `V004-B / F-118`：已确认P2并由后续内存生命周期改造修复（`1a30c01`初修、`9f55542`补强共享持有、`84d910a`收敛为当前typed-route生命周期）；`MediaPreloader` 以 `navigationOwners: [String: Set<UUID>]` 持有同媒体的多个导航 owner，`isTaskRetained` 统一保护导航、焦点候选和附带预载，`ImageNavigationCoordinator` 再以稳定 entry UUID 在Push时取得owner、仅在路径终态移除时释放。manager层 `testPopReleasesTaskOnlyAfterLastNavigationOwnerLeaves` 与生产接线层 `testSameMediaUsesIndependentRouteOwnersAcrossStacks` 分别覆盖最后owner释放和双栈typed-route交接；当前分支独立复核确认原ownerless pin根因已不存在。真机push/Tab/focus表现仍只作为运行未验收边界，不影响代码修复状态。
- `V004-B / F-119`：已确认P2，用户决定跳过（2026-09-17）；当前`MediaInfo.id`仍允许同一`apiMediaId`形成多个任务，`findTask(byMediaId:)`仍只返回第一个，内存生命周期收紧只减少并存数量，导航owner+焦点候选/附带预载仍可同时持有。配套Web v3.0.1没有first-match任务根因，使用canonical身份+season共享缓存，但每个已挂载卡片仍持有独立非响应式`isSubscribed`，mutation只改当前实例和普通Map缓存，其他已挂载同媒体实例也可能保留旧标签；用户据此决定不做TV单端增强。点击时fresh lookup仍限制错误mutation。
...
- `I006 / F-233…F-238`：受限集成与G04末裁闭合；F-233/F-234/F-235/F-236均P2、F-237驳回。F-238确认TV会保留`api_path`既有query并追加筛选值，但配套Web v3.0.1同样以原路径加Axios params构造重复键，标准Starlette/FastAPI标量读取取最后值（筛选值生效），只有插件显式多值/自定义取首解析才有不同；用户决定跳过TV单端处理（2026-09-17）。最终报告永久披露无法取得严格零暴露文件集成票。
...
- `V009-E/F / F-136`：未验证 P3；TV Share 初始/切源默认 count，而本地 v2.15.1 Web 与测试默认 time，差异已确认但 TV 产品意图缺失；用户决定跳过修复，保留该差异和未验证边界，不再安排产品意图确认。
- `V011-A/B / F-137`：已修复（2026-08-18）；四类带宽互不重叠且长度罚分封顶，顺序匹配 fzf 风格词首/连续加分；另按用户裁决新增热度加权（订阅分享固定 0.6 参与加权，多来源混池时全部不计算热度）；SearchViewModelTests 28/28 通过。
...
- `W013-C / F-190`：已修复（2026-09-15）；季卡与详情Sheet共用 `SeasonDisplayFormatter`，统一S00/正季/缺失或非法季号及空白name/date/overview；定向3/3、反向还原失败校验、恢复后3/3与Simulator clean build通过，真机/VoiceOver与真实payload频率仍未验收。
- `W013-C/W015 / F-191`：已修复（2026-09-15）；两个Sheet海报外层由width-only改为固定360×540，继续复用当前processor、clip和占位，不抽组件；定向SubscribeSeasonContentViewTests 1/1与tvOS Simulator clean build通过，本次未跑兼容测试；真实四态渲染及焦点/VoiceOver影响待运行验证。
- `W011 / F-186`：已确认P2；TV从数值倍率重算促销并把合法30/70/25/75压成50、4X压成2x，筛选值与卡片及当前Web/后端`volume_factor`枚举分裂。删除重算helper并复用现有原始字段，以完整枚举表驱动，不建促销模型。；F-186 已修复（2026-08-20）
- `W011 / F-187`：已确认P2；资源业务error、transport失败与成功空都会进入无action空态并由hasSearched阻止同页面重试。复用EmptyDataView action和现有cancelSearch→search即可，不建恢复状态机。；F-187 用户跳过（2026-08-20）
...
- `W014 / F-195`：已确认P2；当前后端按LF拆分多条custom_words且Web使用textarea，TV单行输入无法创建/可靠审阅第二条规则。仅该字段使用tvOS多行控件并保留LF原值，不改全部文本字段。；F-195 用户跳过（2026-08-20）
- `W014 / F-170`：已确认P2；历史停用站点或删除/失权规则组脱离options后不可见、不可清且继续PUT，当前后端可回退默认站点或规则fail-open；只显示并允许主动清域外值，不自动求交。；F-170 已修复（2026-08-20） |
...
- `W015 / F-194/CHK-009`：已确认P2；POST立即持久化keyword/custom_words，但最终确认页不展示。最小只读显示这两个非空字段并支持长/多行可达，其他字段待产品证据。；F-194 用户跳过（2026-08-20）
- `W015 / 既有传播`：401续登后权限撤销仍重放写动作令F-027升级P1并补CHK-005；成功不发`.subscriptionDidUpdate`归F-008，旧错误归F-121，长文本归F-185，海报F-191已修复（2026-09-15）；F-164 已修复（2026-09-15），旧系统实际渲染/焦点仍未单独验证。
...
- `W016 / F-198`：已修复（2026-08-21）；当前后端明确nil为未获取且Web如此展示，TV改为nil显示“未获取”，0/正数原样。StatusDashboardSnapshotTests 投影回归通过。
...
- `V010 / F-139`：已修复（2026-08-21）；Recommend由ContentView传入Tab选中状态并在`.task(id: isSelected)`的激活边沿调用现有恢复入口，详情/合集保留既有成功空恢复；View接线与ViewModel回归均通过。
...
- `I010 / F-020/F-174/F-239及传播`：第三裁及后续G03纠偏闭合；F-020条件P1、F-174升P2且poster/session仍留F-123、F-239 P2，其余传播不变。；F-174 用户跳过（2026-08-20）
...
- `I016 / System整文件`：G06完成最后等级裁决，F-089按当前后端401转确认P2；F-106/F-111/F-112/F-240 P2、F-242 P3且用户决定跳过（2026-09-16）、F-241已由`05ba3bd`修复，F-208为P3且用户决定跳过（2026-09-15），受限集成闭环。严格零暴露票不可能取得并须永久披露。
- `G01 / 第一轮全局回溯`：纠偏及G04交叉裁后，F-244机制并入升级后的F-130/CHK-005并驳回重复编号；F-076/F-129/F-138/F-139等级已收口，其余分歧继续由G05/G06/窄裁处理。
- `G02 / 第一轮全局回溯`：主审完成；无新编号，F-175/F-171吸收MediaCard语义，MediaPreloader当前不成案。F-014当前代码反证及多项P级/范围修订与既有裁决冲突，全部等待不同代理复核，不先覆盖历史两票。；F-171 用户跳过（2026-08-20）；F-175 用户跳过（2026-08-20）
- `G03 / 第一轮全局回溯`：首轮对象错位意见已冻结并经纠偏/窄第三裁排除；最终F-245独立确认P2并与F-083仅共同挂CHK-017，F-219继续驳回，F-097/F-116/F-118/F-171/F-174/F-221均收敛P2，组已闭环。；F-171 用户跳过（2026-08-20）
- `S005/C018-B/W011 / F-110`：已确认P2并修复；G05两名代理再次闭合default方向控件/箭头会切换但比较器忽略`isAsc`的稳定反例。已按产品选择把 `SortType` 扩展为“默认排序/升序/降序”三态：默认排序保留后端顺序且无箭头，默认字段选显式方向时按 `pri_order` 排序；不建排序抽象。
