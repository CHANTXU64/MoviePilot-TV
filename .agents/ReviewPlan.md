...
| 单元 | 状态 | 候选/备注 |
| --- | --- | --- |
| M001-A | 已闭环 | F-006已确认P2；F-007确认P1且已修复（`bb07772`）；F-008确认P2且已修复（`789e9a7`）；F-012按当前七字段合同确认投影缺失/优先级反转P2并已修复（`58c7e81`），canonical-only TMDB group另留未验证P3边界；F-013经当前Web/后端合同反证后驳回，用户决定跳过修复；H-001/H-004 已由 I001 修正闭环 |
...
| M001-J | 已闭环 | G09 clean-room窄裁将F-073收窄后转确认P2；F-074/F-075 P2；F-076原跨owner P1链已闭合，手动媒体ID子项由`44908c4`修复，聚合Search/Resource仅剩同会话陈旧状态P2；扩展F-027下游证据不变 |
| M001-K | 已闭环 | F-080/F-081/F-085均确认P2；F-081 输入边界修复已完成（`670cf86`），验证及独立复审通过，已选规则缺失继续静默不过滤由用户裁为产品取舍；F-085 经 S005 不同代理修订跨端边界，F-061后续由I011升P2 |
| I001 | 已闭环 | 55 个顶层声明全部覆盖；无新增发现，维持既有裁决；四处仅 doc comment 跨账面边界 |
| A001-A | 已闭环 | G02末裁将F-082升条件P1；F-083/F-084/F-030 P2，其余传播闭合 |
...
| S005 | 已闭环 | 主审与独立复核已闭环；F-060/F-061/F-081/F-085 维持，F-017 未验证；下游F-110后经C018/W011确认并由G05升P2 |
...
| V018 | 已闭环 | review_a001_j主审、verify_a001_h独立复核完成；W014补强后F-147/F-148均确认条件性P1；F-147的Subscribe竞跑已由`a872737`修复，F-148及F-147的整理P2传播仍开放 |
| V019 | 已闭环 | 双审完成；G09两票按同权限跨session发布将F-149升P1，F-150维持P2，F-070由未知能力入口升确认P2；混合运维快照与伪空卡边界不变 |
...
| W018-B | 已闭环 | 双审与G09完成且既往源码/ledger暴露永久披露；clean-room窄裁将F-073收窄为data/item缺失fail-open并确认P2，F-151 P1且用户按当前官方Web v2对齐决定跳过，F-162/F-165 P2，其余intent/logID边界不变 |
...
- `M001-K/S005 / F-085`：matcher 与设置预览消费同一 canonical rule；已选规则的空白、坏语法或运行路径不支持必须显式失败，不得在预览显示生效时静默 fail-open/closed。
...
- `M001-K/S005 / F-080/F-081/F-085`：三项均已确认；F-085 已由不同代理修订跨端定性，目标部署后端路径/版本与 System 预览测试继续留 V002/W020/I003/I004/I016/G01/G05 回溯。
...
- `V018/W014 / F-147`：Subscribe P1子项已由`a872737`修复；保存中禁用取消，系统返回当下同步冻结saving状态并跳过回滚，只有该返回路径最终保存成功才发一次全局成功提示；整理Sheet的提交中关闭P2传播仍开放。
