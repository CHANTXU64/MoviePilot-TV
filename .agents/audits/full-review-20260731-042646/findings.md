...
| F-122 | 部分修复 | P3 | V005 | `APIService.recognizeTmdbId:2255-2263`、`MediaActionHandler.swift:31-66`、`HomeView.swift:277/295` | nullable 结果把最终无匹配、失败与取消统一呈现为未识别 | 闭合「首段失败 + 兜底成功但无可用 ID」的残留折叠；Home 真 no-match 是否仍提示属产品意图 | 两段 error/cancel 已 throws；本轮补齐首段失败暂存错误的最终抛出 | 新增 2 条用例 + 反向验证 7 条阴性对照；全量 959/959 通过、零失败 | 🆕 错误折叠已全部闭合；仅剩「Home 标题回退是否仍弹提示」待用户裁决 |
...
- 🆕 剩余待用户裁决（产品意图，非缺陷）：Home 的「搜索资源」按钮（`HomeView.swift:295`）在**真 no-match** 时仍会走到 `else` 分支用标题兜底导航，而此时 `getTMDBJumpTarget` 已置 `showTMDBNotFoundAlert = true`，全局弹窗（`ContentView.swift:105` → `MediaActionModifier`）随之弹出 —— 即同一动作同时产生 alert 与 navigation。改动前该提示是**误导**的（网络错误也弹），现已不再由错误触发，剩下的提示内容是**真实**的（「未识别到此媒体的TMDB信息，已按标题搜索」）。审计原文即注明「Home 对真正无匹配是否仍需信息提示属于产品意图，不能由静态审计代定」，故本轮不擅自改动，留待用户决定。