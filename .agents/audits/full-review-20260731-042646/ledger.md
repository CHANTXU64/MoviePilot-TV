...
- 每批代理须报告除当时的 `.agents/ReviewPlan.md`（现归档为同目录 `review-plan.md`）与本审计目录之外的工作树变化；生产范围变化会重新打开受影响单元。
...
| 审查单元 | 范围/符号 | 依赖层级 | 主审代理 | 主审状态 | 复核代理 | 复核状态 | 发现编号 | 回溯依赖 | 最终状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| V001 | `NotificationManager.swift` 全文件 | L2 | review_a001_j | 已闭环 | verify_a001_h | 已闭环 | F-107原P1主触发已修复，剩余P2由用户决定跳过；F-108 未验证、用户决定跳过修复；传播 F-049/F-060/F-091/F-093 | G08 | 已闭环 |
...
| V004-B | `MediaPreloader.swift:300-474`，全局缓存、Pin、刷新与 LRU | L2 | verify_a001_h | 已闭环 | review_a001_h | 已闭环 | 后续G03将F-118确认P2，现已由`1a30c01`初修、`9f55542`/`84d910a`补强至当前typed-route生命周期；F-119 P2；支持F-004/F-008/F-019/F-020/F-027/F-100及F-115…F-117传播 | G02/G03 | 已闭环 |
...
| V009-E | `ExploreViewModel.swift:561-753`，派生筛选、路径与查询构建 | L3 | verify_a001_h | 已闭环 | review_a001_j | 已闭环 | F-134 未验证待部署；F-136 未验证且用户决定跳过修复；确认 F-129/F-131/F-132；F-088 动态 query 扩展及既有传播 | G01/G04/G05/G06 | 已闭环 |
| V009-F | `ExploreViewModel.swift:754-957`，分页、加载、来源刷新与重置 | L3 | verify_a001_h | 已闭环 | review_a001_j | 已闭环 | 无新编号；确认 F-129/F-130 与 F-132 同根共有值扩展；传播 F-027/F-033/F-035/F-077/F-078/F-082/F-088/CHK-005；F-133/F-134 未验证；F-136 未验证且用户决定跳过修复，F-135 已确认 P3 | G01/G02/G04/G05/G06 | 已闭环 |
| V010 | `RecommendViewModel.swift` 全文件 | L3 | review_a001_j | 已闭环 | verify_a001_h | 已闭环 | 后续G01/G04将F-138升条件P1并由`ff4ea14`修复、F-139升P2；其余推荐分页/session/图片/动作传播不变 | G03/G04/G06 | 已闭环 |
...
| V012-A | `MediaDetailViewModel.swift:1-255`，详情、分页、背景与预加载 | L3 | review_a001_h | 已闭环 | review_a001_j | 已闭环 | 无新编号；F-100/F-130/F-139扩展确认；F-138 task/season/lifecycle alias成立但wrong fullDetail注入收窄未验证；后续G03将F-116/F-118确认P2，F-118现已由`1a30c01`初修、`9f55542`/`84d910a`补强 | G03/G04/G07 | 已闭环 |
...
| C009-B | `MediaCard.swift:228-425`，卡片主体 | L4 | review_a001_h | 已闭环 | review_a001_j | 已闭环 | F-172已修复（2026-09-17）：已知类型保留专用图标，未知/nil/状态文字统一`photo`，MediaInfo入口统一`displayTypeText`；F-173代码级重复处理确认但实际影响未量化，用户决定跳过修复；整卡语义并入F-171，确认F-105/F-106/F-138/F-019/F-020/F-026/F-084传播，F-114/F-158/F-169不适用 | G03 | 已闭环 |
...
| C012 | `BestResultCard.swift` 全文件 | L4 | review_a001_h | 已闭环 | review_a001_j | 已闭环 | F-178条件性P3且用户决定跳过修复：评分备用名与卡片展示名分裂；F-076/F-172/F-174/F-177及搜索评分/身份传播闭合，F-171/F-175不适用；固定高度/完整overview仅留运行盲点 | G01 | 已闭环 |
...
| C018-C | `TorrentsResultView.swift:348-462`，筛选 UI | L4 | verify_a001_h | 已闭环 | review_a001_j | 已闭环 | 无新编号；F-110/F-057/F-058/F-059/F-179/F-168/F-170传播，F-061根在A段；F-163用户决定跳过修复、F-165有反证，筛选身份/禁用移除/focus/accessibility边界闭合 | G05/G10 | 已闭环 |

### 4.4 业务 View 与 Sheet

| 审查单元 | 范围/符号 | 依赖层级 | 主审代理 | 主审状态 | 复核代理 | 复核状态 | 发现编号 | 回溯依赖 | 最终状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| W001 | `ManualMediaSearchSheet.swift` 全文件 | L5 | verify_a001_h | 已闭环 | review_a001_j | 已闭环 | 无新编号；F-076/F-099/F-172/F-158/F-165传播，F-178已确认且用户决定跳过修复，F-177已修复但真机性能/像质仍未验收，F-060/F-157/F-159/F-171/F-175/F-174不适用；AddDownload media_in分工仅留契约边界 | G01/G09/G10 | 已闭环 |
...
| W008-C | `MediaDetailView.swift:588-842`，Hero 与详情内容 | L5 | review_a001_h | 已闭环 | review_a001_j | 已闭环 | F-183已修复（2026-09-16）：详情页TMDB动作入口保留预识别disabled并同步防重入，不新增任务期间disabled，定向测试1/1、反向校验与clean build通过，未跑兼容测试；I013新增F-231确认P2；F-123/CHK-005及Hero/详情投影/图片转场传播闭合 | G01/G03/G06/G07/G10 | 已闭环 |
...
| W013-C | `SubscribeSeasonView.swift:429-506`，详情 Sheet | L5 | review_a001_h | 已闭环 | verify_a001_h＋review_a001_j | 已闭环（既往源码暴露披露） | 长overview并入F-185；F-190已修复（2026-09-15）：季卡与详情Sheet共用SeasonDisplayFormatter，定向3/3、反向还原失败校验、恢复后3/3与Simulator clean build通过；F-191已修复（2026-09-15）：详情Sheet海报外层固定360×540，定向1/1与Simulator clean build通过。无写操作owner；真实四态海报形态、焦点/可访问性与既往源码暴露维持运行盲点 | G02/G10 | 已闭环 |
...
| W015 | `ForkSubscribeSheet.swift` 全文件 | L5 | verify_a001_h | 已闭环（既往源码暴露披露） | review_a001_j | 已闭环 | G06将F-193升条件P1，F-194 P2；F-191已修复（2026-09-15）：Fork海报外层固定360×540，定向1/1与Simulator clean build通过；F-027/CHK-005与F-008/F-121/F-185传播闭合，F-164 已修复（2026-09-15），真实四态渲染/焦点仍未单独验证 | G02 | 已闭环 |
...
| W018-A | `ReorganizeSheet.swift:1-380`，表单 | L5 | review_a001_h | 已闭环（既往局部暴露披露） | review_a001_j | 已闭环（外围入口/ledger暴露披露） | G09后F-188/F-189/F-156为P1，F-147本段P2、F-206 P2；F-075/F-074/F-162/F-168确认，F-163未验证且用户决定跳过修复、F-164 已修复（2026-09-15）、F-166驳回 | G01/G09/G10 | 已闭环 |
...
| I005 | `MediaPreloader.swift` | V004-A…B | review_a001_h | 已闭环（曾独立复核V004-A/B及大量详情调用链，永久披露） | F-221/F-115 P2、F-220驳回；后续I013/G02/G03将F-180/F-118/F-119升P2、F-184升条件P1，F-118现已由`1a30c01`初修、`9f55542`/`84d910a`补强，F-117维持P3；其余传播校准 | G02/G03 |
| I006 | `ExploreViewModel.swift` | V009-A…F | review_a001_h＋review_a001_j＋verify_a001_h | 受限已闭环（现有三代理均曾主审某分段，不能宣称严格独立） | F-233/F-234/F-235/F-236 P2、F-237驳回；F-238后续经配套Web与FastAPI标量合同复核由用户决定跳过（2026-09-17）；G04 clean-room末裁确认path无唯一合同，程序限制不变 | G01/G04/G05/G06 |
...
| I008 | `MediaDetailViewModel.swift` | V012-A…C | review_a001_j | 已闭环（既往详情/订阅调用链暴露永久披露） | F-007 P1、F-182 P2；后续G03窄第三裁确认F-118 ownerless pin根因P2，现已由`1a30c01`初修、`9f55542`/`84d910a`补强且保留真机运行边界，F-047/F-048/F-049/F-130不变 | G02/G03/G04/G07 |
...
| I016 | `SystemView.swift` | W020-A…H | review_a001_h＋verify_a001_h＋rounda_g01_recheck＋rounda_g02_third | 受限已闭环（参与代理均有W020/G01或其他会话组暴露，不能宣称严格独立） | G06将F-089转确认P2；F-106/F-111/F-112/F-240 P2，F-242 P3且用户决定跳过TV端处理（2026-09-16），F-208 P3且用户决定跳过（2026-09-15），F-241已由`05ba3bd`修复；其余传播不变 | G01/G05/G06/G10 |
...
| B006-A / F-037 | A001-I、V004-A、V012-A、W008-C、I001/I002/I008/I013、G07 | 有效语言标签未经规范化而退化为原码 | 上游格式、BCP 47/别名支持边界 | 已闭环；用户决定跳过修复，保留未验证状态 |
...
| B006-B / F-042 | M001-C/D、A001-I、W008-C、I001/I002/I013、G07 | 国家对象/字符串形态没有统一 alpha-2 规范化 | 上游形态与 alpha-3/别名边界 | 已闭环；按用户裁决修复 trim + 大写归一，定向16/16并完成反向验证；全量因用户要求跳过兼容测试而未完成 |
...
| V001 / F-108 | C002/R001、V017/W013、V022/W019、G08 | 根通知可能在独立 Sheet 下不可见却照常计时并过期 | tvOS 模态层级运行证据、延后消费或可见时计时边界；用户决定跳过修复，保留未验证运行边界 | 已闭环 |
...
| V009-E / F-136 | V009-F、W004、I006、G01/G05 | Share 初始/切源默认 count，与 v2.15.1 Web 默认 time 相反 | TV 产品意图、两处 literal、首请求路径与默认 Picker 回归 | 未验证；用户决定跳过修复，保留 TV 特例/产品意图边界 |
...
| C004 / F-163 | C006/C008、W012/W014/W018-A、G10 | tvOS 26.0–26.3自定义Button/Toggle样式忽略isEnabled，禁用控件与启用未聚焦外观相同 | 现有样式读取Environment isEnabled并统一降低不可用态对比度；需目标OS视觉验证 | 未验证；用户决定跳过修复，保留运行边界 |
...
| C009-B / F-172 | C013、各MediaCard调用页、G03 | nil/空/未知typeText的缺图占位统一回退film，电视剧订阅状态文本和季卡均被误标电影 | 已修复：共享`MediaTypePresentation`保留movie/tv/collection/person专用图标，未知类型统一`photo`，MediaInfo调用点复用`displayTypeText` | 已闭环 |
...
| C012 / F-178 | I007/W006-D/W001、G01 | 评分消费备用名称而Search/Manual卡片只展示主名称，最高分有效结果可为空标题或“未知” | 用户决定跳过修复；Web前端没有TV的本地bestScore/最佳结果聚合链，暂不做TV单端展示fallback；真实备用名payload频率未验证 | 已确认（用户决定跳过） |
...
| W008-C / F-183 | G03/G06/G10/I013 | TMDB按钮在创建Task前无同步reentry owner，两个调用可重复append且先完成者提前清共享busy | 已修复（2026-09-16）：保留预识别期间原有disabled，动作入口同步设置本地in-flight，重复激活直接返回；成功/失败/取消均清除，离开route后的单动作晚到仍由F-231 P2承载 | 已闭环 |
...
| W013-C / F-190 | G02/G10 | S00缺名在详情显示“第0季”而卡片显示“特别篇”；空白name/date/overview生成空标题、图标空行或空壳区域 | 已修复：季卡与详情Sheet共用SeasonDisplayFormatter；定向3/3、反向还原失败校验、恢复后3/3与Simulator clean build通过；真机/VoiceOver与真实payload频率仍待补 | 已修复（2026-09-15） |
| W013-C / F-191 | W015、G10 | processor按360×540降采样但外层width-only；缺图/失败只剩无2:3固有高度的Rectangle，四态无法保证稳定几何 | 两个Sheet直接固定360×540，覆盖URL缺失/loading/失败/成功；定向1/1与Simulator clean build通过，真实右栏/焦点/VoiceOver仍待运行验证 | 已修复（2026-09-15） |
...
| G08 / F-107/F-108/F-121/F-159/H-012 | V001/C002/C016/W015/R001/R002、F-049/F-148 | F-107主触发已修复、剩余P2由用户决定跳过；F-148 P1，F-049/F-121 P2；F-108未验证P3且用户决定跳过修复，F-159运行P3。G02末裁覆盖F-121旧P3 | H-012只传播；各运行边界保留明确验收 | 已闭环 |
...
| G07 / F-045/F-050/F-051/F-055/F-056/F-178/F-228 | B005/B006/S006/V012/W006/W008/W009 | 三方确认Hero、Douban roles、两类头像与详情备用名各自投影边界；job canonical正常，mergeCrew非空existing当前不可达 | F-045/F-050/F-051/F-055维持P3，F-056驳回重复并入F-050，F-178用户决定跳过修复，F-228独立确认P3且用户决定跳过TV端修复（2026-09-16）；复用displayRole/imageURLs/alternateNames | 已闭环 |
...
| G10 / Sheet焦点与样式全局链 | C003/C004/C005/C017及全部业务Sheet、W013/W015/W018/W019/W020 | ActionRow、SheetStyles/TextField/Picker、MultiSelection与业务Sheet的焦点、控制语义、Exit、嵌套呈现、状态反馈、长文本及VoiceOver须全局收敛 | 双审闭合；F-229已修复（2026-09-16）：即时生效多选按钮改为“完成”，定向测试1/1与clean build通过，未跑兼容测试；F-230确认P2，既往多业务Sheet调用链污染永久披露 | 已闭环 |
| G10 / mutation与presentation映射 | W013/W015/W017/W018/W019、F-092/F-108/F-120/F-147/F-148/F-193/F-205 | mutation、Cancel、single-flight、嵌套Sheet及焦点恢复须复用既有operation/session owner | F-120后续降P2且用户决定跳过；G06将F-193升条件P1，F-148/F-027 P1、F-205 P2，F-108未验证P3且用户决定跳过修复 | 已闭环 |
| G10 / query/style/focus/text/permission映射 | C003/C004/C005/C017、W013/W018/W020、F-076/F-126/F-130/F-158/F-160/F-162…F-165/F-168/F-185/F-229/F-230 | query generation、disabled/empty、初始焦点、原生控制、长文本和权限快照须全局一致 | 后续G09将F-161/F-162/F-165升P2；F-158/F-160/F-168/F-230 P2；F-229已修复（2026-09-16） | 已闭环 |
...
| I016 / F-242 | W020-D/E、G10 | 站点/规则长名单行截断且preview不回显完整名称 | 两票确认视觉P3；用户决定跳过TV端处理（2026-09-16）；推荐截断与VoiceOver扩大说法保留运行边界 | 已闭环 |
...
| S10 | B006-A 主审与独立复核 | F-037 未验证 P3、用户决定跳过修复；F-038 已确认 P3；分段已闭环 |
| S11 | B006-B 主审与独立复核 | F-042 已修复 P3（仅 trim + 大写归一）；F-043 已确认 P3；分段已闭环 |
...
| S443 | F-221 修复登记 | 用户以大白话报告确认机制后批准修复：`MediaPreloadTask.start()` 只在 partial 快照判一次识别，custom partial（canJumpToTMDB=false）跳过识别且不落定 `isTmdbRecognitionFinished`，full detail 补出 douban/bangumi/anilist 身份（canJumpToTMDB 翻 true）但无 TMDB 时 Header TMDB 按钮因「无目标且识别未结束」永久 spinner。修复：start() 在 `await (tmdbRecognition, detailLoad)` 后按 canonical media（`fullDetail ?? partialMedia`）重评，tmdbId 仍 nil、识别未落定且 canonical 有可识别身份时补调 `recognizeTmdb(using: canonical)` 落定 finished（成功填 tmdbId、no-result/失败/取消均落定）；`recognizeTmdb` 参数化改用 canonical 标题。新增 `MediaPreloadPermissionTests` 2 条回归（custom+detail 补 douban：no-result 落定 / search 命中填 tmdbId），类内 18/18 通过，相关类 MediaDetailViewHeaderAction/DynamicSourceBehavior/SubscribeSeasonContentView/SubscribeSheetViewModel 全过 |
| S444 | F-223 通知呈现处置 | 用户以大白话报告审阅后决定跳过：`NotificationManager` 单槽自动计时下“失败弹条、成功静默”操作在重试成功后无 scope dismiss 撤销自身旧失败（A失败/B失败/A成功不得误删B）。保留历史 P2 结论，但不再列为待处理项；不改动 manager，也不接线任何调用点 |
| S445 | F-225 修复登记 | 用户以大白话报告批准 3 秒方案后落地：`SearchViewModel` 统一搜索为可选“订阅分享”加首屏等待上限（默认3秒、测试可注入）。分享刷新改为与核心四类并发启动且 fire-and-forget，核心四类完成后轮询至多3秒即收口计算 `bestResults`，不再整体 `await` 分享；迟到分享请求不取消，晚到只补“订阅分享”Paginator 行，不回填已收口“最佳结果”行；换词搜索走既有 reset 取消旧分享。新增3条回归测试（慢分享核心先行揭示且最佳行不动+迟到只入订阅分享行；窗口内返回保留原行行为；换词搜索取消防泄漏），`SearchViewModelTests` 整类 32/32 通过；提交 SearchViewModel.swift + SearchViewModelTests.swift + 台账 |
| S446 | F-226 用户决定跳过登记 | 用户以大白话报告确认“仅 Bangumi 来源以 career 展示演职职责（TMDB/豆瓣各自角色字段已正常显示、AniList 无真人演员数据）”后决定跳过：不改动 `Person` 解码、StaffManager 同人合并或 PersonCard/搜索卡片投影；保留历史 P2 结论，不再列为待处理项 |
| S447 | F-231 用户决定跳过登记 | 用户以大白话报告（详情页“去 TMDB”动作离场后，未取消、无 route owner 的旧 Task 成功仍 append 共享 path、失败在无关页弹旧提示）审阅后决定跳过：不改动 `MediaDetailView` 按钮 Task 生命周期、导航追加或提示门禁；保留历史 P2 结论，不再列为待处理项 |
| S448 | F-232 用户决定跳过登记 | 用户以大白话报告（根子在 MoviePilot 后端秒级时间分页仅 `date DESC`、同秒无全序可跨页重复/漏项，本仓库为 TV 端无后端修改权、TV/Web 均无客户端补偿）审阅后决定跳过：不改动后端或 TV；保留历史 P2 结论，不再列为待处理项 |
| S449 | F-233 用户决定跳过登记 | 用户要求核对 Web 对照后决定跳过：`MoviePilot-Frontend/src/views/discover/ExtraSourceView.vue` `watch(filterParams)` 同样在“空值且存在默认”时回填默认，TV 现状与 Web 行为一致；用户判断“对齐了即可”。不改动 TV 运行值 owner；保留历史 P2 结论，不再列为待处理项 |
| S450 | F-234 用户决定跳过登记 | 用户核对 Web 对照后决定跳过：`ExtraSourceView.vue` 同样只 `watch(filter_params)`，`options/depends/filter_ui` 变化不清失效值；FormRender 仅 v-model 绑定、无失效自愈，TV 现状与 Web 行为一致。恢复路径已确认：值仅存于内存 `@Published pluginFilterValues`，切走再切回 Discover 源、手动改控件或重启 App 即恢复。不改动 TV；保留历史 P2 结论，不再列为待处理项 |
| S451 | F-235 用户决定跳过登记 | 用户以大白话报告质疑后核实决定跳过：Explore 一次只选一个来源、单 paginator 单 feed，跨源重复从结构上排除（两不同来源永不同屏）；导航/预载走已归一 `identity?.mediaKey`，`popularSubscriptionKey` 仅用于热门订阅 feed 内部去重、`updatedExtraSourceSnapshot` 仅影响选择器 chip，别名双卡最坏只多一张同详情卡。`subscribe/popular` 为外部社区共享订阅统计聚合（`server.py:599` 需 `SUBSCRIBE_STATISTIC_SHARE`，聚合在官方 MoviePilot 统计服务器），别名双卡需上游对同一媒体混用 `tmdb`/`themoviedb` 写法，TV/Web 无法控制且无证据会返回。不改动 TV；保留历史 P2 结论，不再列为待处理项 |
| S452 | F-236 修复登记 | 用户以大白话报告后批准修复：`ExploreViewModel.init` 管线由"投影纯 path → `removeDuplicates()`"改为"携带 `(sourceID, path)` → sink 内按 `sourceID\0path` 字符串键手动去重"（元组不满足 `Equatable` 协议无法直接入链，故去重下移 sink、键仅 `String?`，未建 owner 类型/状态机，path 原样交 `setupPaginator(for:)`）；同一 (sourceID,path) 仍跳过、换源或换路径必重建，重建时读新 `selectedSource` 以新源语义建 Paginator（fetch/decode/processor/seenKeys 随实例重置）。新增 `ExploreViewModelPaginatorOwnerSwitchTests`（两 custom 源 api_path 完全一致、prefix 不同，断言切源后 Paginator 实例重建），对旧实现反向验证确实失败（2.6s XCTFail）、修复后通过；PaginatorOwnerSwitch 1/1、ExploreViewModelTypeSwitch 7/7、ExploreViewModelYearDict 3/3、DynamicSource + MediaInfoCollection 56/56 全绿。提交 ExploreViewModel.swift + ExploreViewModelPaginatorOwnerSwitchTests.swift + 台账 |
| S453 | F-239 用户决定跳过登记 | 用户提示"P1 修改好像修了"后核实决定跳过：跨账号防线确已存在（`MediaPreloader` 监听 `apiService.$session`，登出/换账号即 `clearAll()` 清已登记任务），但 Search `ResultRow`/`BestResultRow` 各自 `@State` 持有的 300ms 睡眠裸 Task 尚未登记、不受 clearAll 管；`SearchView.swift` 全文无 `.onDisappear`，`MediaPreloadTask.start()` 开头无 canAccess/会话门禁，故"聚焦后 300ms 内非点击离页"仍漏发一次请求，极端时序（登出+登新号发生在 300ms 内）才跨账号漏发。实际代价为一次多余/必然 401 的请求（结果进缓存自愈），非错数据级。不改动 TV；保留历史 P2 结论，不再列为待处理项 |
| S454 | F-240 修复登记 | 用户先追问"后端和 Web 到底会不会返回同名"后批准修复。核实：后端官方仓库无任何 `RecommendSource` 事件 producer（`app/` 内仅枚举/读取端点/schema/`fetch_medias` 发送方，默认 `/recommend/source` 返回 `[]`），extra 仅来自第三方插件且 `name` 自由文本、无唯一约束，Recommend 描述符无 `mediaid_prefix` 类稳定键；Web `recommendSources.ts` 合并同样只按 apipath 去重、`recommend.vue` 配置同样 title-keyed（注释"以title为key"），与 TV 同病而非"不会同名"的证据。修复：`RecommendViewModel.enableConfig` 寻址全程由 `shelf.title` 改稳定 `shelf.id`（filteredShelves/新增静态 `enabledShelves`/visibleCategories/init 默认/refreshSources AniList 默认均按 id），SystemView 推荐 Toggle get/set 用 `shelf.id`；新增 `migrateTitleKeys(in:shelves:)` 一次性迁移——id 键保留、唯一 title 改写保留值、同名歧义平铺各同名 id 供拆分、未知键不删；`loadConfig` 迁移后 id 键回写落盘、`refreshSources` 拉回 extra 后二次迁移。更新 `RecommendCategoryVisibilityTests` 随契约改 id 键（原测试钉旧 title 契约）；新增 `RecommendShelfToggleKeyTests` 5/5（同名不同 path 独立开关、分类可见性跟随各自 id、旧 title 迁移保留值并回写、helper 歧义/未知边界、SystemView 接线守卫）；RecommendCategoryVisibility 2/2、SuccessEmptyReactivation 3/3、DynamicSourceBehavior/SystemViewDefaultStyle/BackendCompatibilityTests 全绿。提交 RecommendViewModel.swift + SystemView.swift + 两个测试 + 台账 |
| S455 | F-243 修复登记 | 用户两次追问场景澄清（"没太看懂"→大白话重讲后台媒体库变化如何进入新建订阅默认、"就是说进APP时再重新刷新一次"→澄清为回前台而非冷启动）后批准修复。修复：`SubscribeSeasonView.swift` scenePhase `.active` 处理（原只 `checkSubscriptionStatus(forceRefresh:true)`）改为先 `await viewModel.checkSeasonsStatus()` 再刷订阅。`checkSeasonsStatus()` 为既有公开方法，自带 session/剧集组 scope 守卫、成功后整体替换、失败/取消保留旧快照并恢复 scope，与弹窗关闭回调同源（`SubscribeSeasonView.swift:365-367`）；可用性刷新后 `prepareSubscription` 读取的 `seasonsNotExisted` 与当前媒体库一致，`best_version`/`best_version_full` 不再基于陈旧值进入 create→pause 的 `addSubscription` POST（`Subscribe.addRequest` 原样携带）。验证：新增 `SubscribeSeasonForegroundRefreshTests` 接线回归（切片 scenePhase `onChange` 块到其首个订阅刷新，断言含 `guard phase == .active` 且 `checkSeasonsStatus()` 在前）；既有 `SubscribeSeasonContentViewTests` 64 项已覆盖 `checkSeasonsStatus` 刷新语义（成功整体替换/同账号 token 刷新取消保留旧值/失败保留旧角标/切号清除），本次只补"前台接没接上"一环。本项新测试 + SubscribeSeasonContentViewTests + SubscribeSheetViewModelTests 全绿。提交 SubscribeSeasonView.swift + SubscribeSeasonForegroundRefreshTests.swift + 台账 |
| S456 | F-245 修复登记 | 用户以大白话报告核对后端/Web fail-closed 合同后批准修复。核实：TV `forkSubscription` 原用 `success != false`（fail-open），与本仓 `decodeStrictActionResponseSync` 的 `success ?? false` 及 Web `ForkSubscribeDialog.vue` 的 `if (result.success)` 不一致；后端 `schemas.Response.success` 为必填 bool 且 `create_subscribe` 返回 `success=bool(sid)`——当前后端不可能发出缺失/null success 的 Fork 成功响应，故纯失败关闭加固、对真实后端零行为变化。修复：`APIService.forkSubscription` 两处守卫收紧——`response.success == true`（缺字段/null/false 一律抛 `复用订阅失败`，服务端 `message_i18n/message` 透传）且 `data.id > 0`（缺 id/data null/id 为 0 或负数抛 `复用订阅响应缺少 ID`）；只改本端点，不重构下载等其它 decoder。验证：新增 `ForkEnvelopeStrictnessTests` 矩阵——`success` 显式 true/false/null/缺失 × `id` 正常/缺失/null/0/负数/自相矛盾（false 却带正 id）＋空对象共 10 格，仅 success==true 且 id 为正返回，其余 9 格断言抛对应 `serverMessage` 且文案透传正确；新矩阵 + `ForkOperationOwnerTests` 4/4 + `PermissionBehaviorTests` + `APIServiceCompatibilityEndpointTests` 全绿（后两套既有 stub 均显式带 `success`，与 fail-closed 兼容、无旧行为依赖）。提交 APIService.swift + ForkEnvelopeStrictnessTests.swift + 台账 |
| S457 | F-001 修复登记 | 用户在 P3 逐条过时先质疑"F-001 不是已经修了吗"，经 `git blame` 核实整个 `FlexibleBool` struct（Models.swift:187-224）仍为 2026-03-06 初始提交 `a37abbf` 原样、全仓仅此一处定义且无 `.whitespacesAndNewlines` 落入该分支，确认未修后批准修复。触发为任一 `FlexibleBool` 字段收到带行尾字符串（`"true\n"`/`"1\r\n"`），`.whitespaces` 不含 `\n`/`\r`，真值比较与 `Int` 转换均失败后静默落入 `false`。修复：Models.swift 字符串分支 trim 由 `.whitespaces` 改 `.whitespacesAndNewlines`，方向 fail-closed 不变、只把被吞真值恢复为 true，无权限提升风险。验证：新增 `FlexibleBoolDecodingTests`，先用 `JSONSerialization` 以合法 `\n`/`\r\n` 转义构造 payload（直接拼字面换行是非法 JSON、会误测），覆盖 `true/1/yes/on` 与 `false/0/no/off` 各带行尾 8 例；修复版全绿，临时还原旧 `.whitespaces` 后同套 4 例 truthy 全败、确认测试能捕获本缺陷（falsy 因 fail-closed 本就通过）。提交 Models.swift + FlexibleBoolDecodingTests.swift + 台账 |
| S458 | F-004 用户决定跳过登记 | 用户在 P3 逐条过时问"F-004 是啥"，经解释触发/持有链与风险定性后决定跳过。核实：`rawPayload`（Models.swift:629/758/955）在 `MediaInfo`/`MediaInfoJSON` 解码时把整份原始 JSON 转 `[String: JSONValue]` 全量再存一遍，与强类型字段重复持有，长分页/预加载放大；但 M001-C/verify_m001_c 均只有静态证据、无 Allocations/RSS/解码耗时/真机淘汰量化（P2→P3 即因此），且 `Models.swift:758` 注明其唯一生产用途是 re-encode 时逐 key 回写、保留 Web 回传的插件/未建模字段（`media_in` 原样直传）。用户决定跳过：不冒"丢插件契约"风险做无实测支撑的瘦身；若真机 Instruments 量化出实际影响再重开。不改代码；台账记为降级（用户决定跳过）。 |
| S459 | F-005 修复登记 | 用户在 P3 逐条过时问明触发链后批准修复。核实：`StatusViewModel.refreshAllData` 用 `async let` 并发取统计/存储/下载器三接口并 `try await (stat, stor, down)` 一次性取值，任一解码失败整组抛错、三张卡一起不刷新（非只坏一格）；而 `Statistic.movie_count/tv_count`、`DownloaderInfo` 五字段均声明为非可选带 `= 0`，但属性默认值不会成为合成 `Decodable` 的缺键默认值——缺键/null 直接抛错。修复：两模型各补自定义 `init(from:)`，非可选数字字段一律 `decodeIfPresent(Int.self, forKey:) ?? 0`（`Statistic.episode_count` 保持可选），属性 `= 0` 保留为文档化默认；选"字段允许缺失"分支而非"移除默认值+严格契约"，因影响面是整页停滞、且与仓内 F-022/F-023 宽容解码方向一致；字段齐全时解码结果完全不变。验证：新增 `StatusModelDecodingTests`（Statistic/DownloaderInfo 各一组缺键/null/空对象/字段齐全矩阵）+ `StatusDashboardSnapshotTests` 两个端到端用例（stub 只回部分字段时整页仍发布、同批 storage/downloader 不被拖累，为此给该 stub 增 body 覆盖能力与 reset 清理）。修复版 9 例全绿；`git stash` 临时还原模型改动后，4 个新用例全败、既有 5 例不受影响，确认测试能捕获本缺陷。提交 Models.swift + StatusModelDecodingTests.swift + StatusDashboardSnapshotTests.swift + 台账 |
...
| S460 | F-040/F-041/F-045/F-052/F-053 组一修复登记 | 用户在 P3 逐条过时要求"这几条一起摸清"，先按"用户怎么触发、影响多大"逐条说明（共性机制：`StaffManager.mergeCrew` 最终 map 把翻译后的显示文本**写回 `person.job`**，而详情页职员行 `PersonCard` 正是渲染该字段，故 canonical key／去重 key／显示串由同一 String 承担），用户随后指示"你先把组一修复"。核实五条同源：F-040 `Cinematography`+`Camera` 两个不同原始 key 同译为「摄影」、原 key 阶段去重放行 → 职员卡显示「摄影/摄影」；F-041 消费者仅 trim `.whitespaces`（不含 `\n`），大小写/换行变体同时绕过 `jobTranslationMap` 与 `jobPriorityMap`，翻译与优先级对同一字符串得出不同结论；F-045 Hero 的 roles 兜底分支（门禁 `staffGroupedByJob.isEmpty`）显示职位而 `processCrew` 不投影 roles、`PersonCard` 全仓从不读 `roles`，同人两处不一致；F-052 roles 先 `joined(separator:"/")` 再整体查表得 999，Producer 等次要职位可压过含 Director 者（本组唯一**结果错误**而非纯显示问题），空 roles 还造孤立 `/`；F-053 已翻译结果回灌 `existing` 形成「导演/Director」→「导演/导演」（当前无非空 existing 生产调用者，属潜伏缺陷）。修复：`JobRegistry.swift` 新增单一解析入口 `canonicalJobKey(for:)`/`canonicalJobKeys(from:)`（清理 `whitespacesAndNewlines` → 精确匹配 → 大小写不敏感 → **未知原样保真**），供翻译与优先级共用；`TranslationHelper.translateJobs` 改为先规范化再对**翻译结果**去重（原始 key 阶段去重与优先级计算不受影响，且同一去重顺带闭合 F-053 回灌叠加）；`StaffManager.getPriority` 改为拆分后取各项优先级最小值，`mergeCrew` 单批优先级收敛到该入口、去掉逐项 trim 后查表；`mergeUniqueStrings` 与分组拆分的 trim 收敛为 `.whitespacesAndNewlines`；roles 兜底标签逐项规范化后再拼接；`mergeCrew` 最终 map 在 `job` 与 `character` **均空**时把 roles 投影进 `job`（严格贴合 G07 第三裁表述，`character` 非空不覆盖，未越界）。F-053 选"显示边界去重"而非"删除增量语义"，保留 Loadmore API 形状。验证：新增 `StaffJobNormalizationTests` 11 例，覆盖 F-040 单记录双 key 与双记录合并、F-041 大小写/换行/首尾空白/未知保真/全空白与 Hero 排序、F-052 多值取优与空 roles 不造 `/`、F-045 投影与 character 阴性对照、F-053 回灌幂等与既有顺序稳定；修复版 12 例全绿。反向验证分两步：先 `git stash` 三个源码文件确认新 API 缺失导致测试目标不可编译，再补齐 canonical 函数而消费者保持修复前行为作对照——12 例中 10 例失败、2 例通过，通过的正是刻意设置的控制项（`canonicalJobKey` 纯单元测试两侧皆在；`testCharacterSubtitleIsNotOverriddenByRoles` 为阴性对照），证明测试确实捕获缺陷而非恒真。全量套件仅 `BackendCompatibility` 两例失败，均为打真实后端的环境性失败且与本改动无关：`testReadOnlyTVSurfaceCompatibilityAndImageRendering` 首轮因模拟器掉网（`NSURLErrorDomain -1009`／`NWPath unsatisfied`，耗时 554s）失败、复跑通过（393s）；`testAIReorganizeCompatibility` 后端返回 HTTP 403，`git stash` 基线复跑失败文本逐字相同，确认既有失败。其余用例（含本项 12 例）全部通过。提交 JobRegistry.swift + StaffManager.swift + TranslationHelper.swift + StaffJobNormalizationTests.swift + 台账 |
| S461 | F-038/F-043/F-046 组二修复登记 | 用户在 P3 逐条过时要求把翻译/职位/元数据这一族一起摸清，组一（职位 key 规范化）交付后接着修组二。三条缺陷同源：详情页两行元数据由 `MediaDetailView` 内联闭包拼装，判据只到「来源字段/数组非空」，从不到「**产出的显示串是否为空**」，空串与畸形元素被原样 append，最后被外层 `joined(separator: " · ")` 渲染成悬空分隔符（`2024 · 中国 /  · 英语`）或一条空 `Text`；叶子层 `languageName`/`countryName`/`translateGenre` 又都是精确查表不 trim，带空白输入直接绕过词表原样回退。出口有三条：F-038 空白 `original_language`（只判 non-nil）、F-043 畸形国家元素（`ProductionCountry.init` 对 null/数字/布尔/空对象静默解成 `(nil,nil)`，对象入口再回退空串）、F-046 空/带空白类型名（`compactMap { $0.name }` 只丢 nil，空串不是 nil）。修复：新增 `MoviePilot-TV/ViewModels/MediaMetadataText.swift` 承载两行组装，所有元素过 `displayValue(_:)`（trim `whitespacesAndNewlines`，空结果丢弃），`category`/`type`/`release_date`/`year` 四个标量一并回溯；`TranslationHelper` 三个叶子改为先 trim 再查表、全空白返回空串，`countryName(for country:)` 在名称也缺失时**保留未知非空 code 原文**（原先静默丢弃上游信息）；`MediaDetailView` 两处闭包（原 856-872、900-921）替换为 `primaryLine(for:)`/`secondaryLine(for:)` 调用，渲染形状不变。 | 1 | 新增 `MediaMetadataTextTests`（14 例，全部从 JSON 解码 `MediaInfo` 走生产解码路径，断言落在最终拼接结果上而非中间数组）；`-only-testing` 两类 26/26 通过（`** TEST SUCCEEDED **`）。反向验证采用组一的两步法：先 `git checkout` 回到修复前叶子层、再把 `MediaMetadataText` 临时替换为逐字复刻的修复前 builder，13 失败 / 1 通过，唯一通过的是刻意设置的阳性对照 `testAllValidFieldsArePreserved`，确认断言区分的是缺陷本身而非仅引用新 API；随后从 `/tmp` 备份恢复并确认临时垫片残留计数为 0。发现并当场处理：test 输出行被并发 clone 截断，用 `sort -u` 计数时漏统计 1 例，改用 grep 原文核对后确认 12/12 齐跑。环境处置：用户反馈 `xcodebuild test` 每次把 Simulator.app 激活到其工作窗口之上，退出 Simulator.app（模拟器仍 headless 运行）、关闭并行测试（消除 Clone 窗口）、跑前 `simctl bootstatus` 预热设备。全量套件（907 例）回归：906 通过 / 1 失败，失败项为 `testReadOnlyTVSurfaceCompatibilityAndImageRendering` —— 1325s 后在 1720 个真实图片 URL 中有 3 个 Bangumi poster 拉取超时（`NSURLErrorDomain Code=-1001 请求超时`，走 `mp.chantx.top/api/v1/system/img` 代理）；该用例只做后端面扫描与图片可渲染断言，断言里没有 genre/country/metadata 任何一项，也不经过本次改动的两个 builder；且组一提交前同一用例就以另一个网络错误（-1009 断网）失败、单独重跑通过，属环境性。同一轮 `testAIReorganizeCompatibility` 本次通过（16.2s），印证组一时的 403 确为后端瞬时。 |
| S462 | F-051/F-055 组三修复登记 | 用户问「组三是啥」后听完触发链与影响面，决定修。两条是同一事实分歧的两个出口：「这个人有没有头像」有三份判据 —— `StaffManager.hasAvatar` 看任意原始字段存在（`profile_path`/`avatar`/`images` 任一非空）、`SearchViewModel.calculateBestResults` 人物准入看 TMDB 专属 `profile_path`、而卡片渲染只看 `Person.imageURLs.profile`（按 source 严格挑选真正能渲染的 URL，`PersonCard.swift:88`、`SearchView.swift:719` 均已正确使用）。F-051 出口在 `mergeCrew` 新增项同优先级排序：只有占位图的人被判成有头像而排在前面（反例：Bangumi 仅 large、豆瓣默认头像、TMDB 空 images）。F-055 出口在最佳结果准入：豆瓣等来源有可渲染 avatar 但无 `profile_path`，在 `maxS < 50 && pop < 1` 下被当无图低质结果排除，而同一人仍出现在人物行。修复：新增 `Person.hasUsableProfileImage`（= `imageURLs.profile != nil`）作为单一事实来源；排序消费点改用并**删除** `StaffManager.hasAvatar`（全仓唯一调用点）；搜索准入改用同一判据，只动准入布尔量。第三裁保留 P3：F-051 影响限排序，F-055 真实触发需四个条件同时成立（豆瓣来源通常匹配分高），修的是判据本身而非可见频率。 | 1 | 新增 `PersonProfileImageAvailabilityTests`（8 例：4 个叶子反例 + 2 个正向对照 + 1 个排序出口 + 1 个阴性对照），`SearchViewModelTests` 新增端到端 1 例（为此给既有 stub 增加 `setPersonResults(_:forQuery:)` 覆盖 `/media/search?type=person`）。定向 9/9 通过。反向验证：把新判据实现换回修复前的原始字段逻辑（保留名字以便编译）、搜索准入换回 `profile_path`，6 失败 / 3 通过，三个通过的正是刻意设置的对照（AniList 仅 avatar、豆瓣 avatar、职位优先级阴性对照），确认断言咬的是判据本身；随后从 `/tmp` 备份恢复并确认垫片残留为 0。全量套件（916 例 = 原 907 + 本次新增 9）**916/916 通过、零失败**（`** TEST SUCCEEDED **`），组二那轮唯一的失败项 `testReadOnlyTVSurfaceCompatibilityAndImageRendering`（后端图片拉取超时）本轮通过，进一步坐实其为环境抖动。过程记录：全量套件跑到一半时发现自己在 `StaffManager.swift` 删多了一个空行（纯空白改动），按组一同样的标准判定该轮证据作废，杀掉重跑而非带瑕疵结论提交。 |
| S463 | F-044 组四修复登记 | 用户在 P3 逐条过时听完触发链（搜索响应人物含 canonical `job` → 搜索链路完全绕过 `StaffManager.processCrew` 的职位翻译边界 → `PersonCard` 与最佳结果卡片直接把 raw `job` 当副标题渲染）与影响面（中文界面人物行显示英文 `Director`，而详情页同一人显示「导演」）后决定修。该缺陷的边界特征：详情页职员走 `processCrew` 才翻译，搜索结果这条链路另起一路、没有任何翻译出口；即使服务端返回的 job 是完全 canonical 的 key 也照触发，与 F-041 的变体失配无关。修复：在人物分页器 `processor` 统一投影（`SearchViewModel.translatingJobForDisplay`，`:348-357`），用 `TranslationHelper.translateJobs` 翻译非空 `job`，结果为空或与原文相同时原样返回 —— `translateJobs` 对未登记 key 原样返回且翻译后去重，对已翻译值幂等，故重复投影不会叠成「导演/导演」。投影位于 `Person.deduplicate` 之前，首屏与 loadMore 走同一条路径；人物行（`personPaginator.items`）与最佳结果卡片（`bestResults`，由 `calculateBestResults` 从同一批 items 派生）同源，一处修复覆盖两个出口，不在两个 View 各打补丁。 | 1 | `SearchViewModelTests` 新增 4 例（复用组三为既有 stub 加的 `setPersonResults(_:forQuery:)`）：`testSearchPersonJobIsTranslatedForPersonRow`（canonical `Director` → 「导演」，断言人物行）、`testSearchPersonJobIsTranslatedForBestResultSubtitle`（最佳结果卡片副标题同为「导演」）、`testSearchPersonJobProjectionHandlesMultiJobAndMissingJob`（`Director/Writer` → 「导演/编剧」；无 `job` 的人物不被凭空造职位、`character` 原样保留）、`testSearchPersonJobProjectionIsIdempotentForTranslatedValue`（已翻译值「导演」原样保留，阴性对照）。定向 4/4 通过。反向验证：保留 `translatingJobForDisplay` 定义、只把 processor 里的调用还原为 `newItems`（修复前行为），3 失败 / 1 通过，唯一通过的是刻意设置的幂等阴性对照，确认断言咬的是投影本身而非仅引用新 API；随后恢复并复跑 4/4。全量套件（920 例 = 原 916 + 本次新增 4）**920/920 通过、零失败**（`** TEST SUCCEEDED **`）。 |
| S464 | F-057/F-058/F-059 季集解析族处置登记 | 用户在 F-044 后要求先说明这一族，听完触发链与影响面后指示「修那一行，其他跳过」。三条同源：只有 `ParsedSeason` 一套自造正则、唯一消费者是 `TorrentsResultView.swift:180` 的「季」筛选下拉排序，点击筛选走原始字符串精确匹配（`contextMatches`），故错误只落在**选项顺序/归类**上。F-057 正则捕获第二季 `-S(\d+)` 却从不读取（`S01-S12` 当第 1 季排到最后），且集范围用终点、季范围不用的内部意图不一致。F-058 卡片美化器（`Formatters.formattedSeasonEpisode`）与筛选排序器是两套语法，卡片认 `E01-E05` / `S01-02 E03-04` 而 ParsedSeason 强制 `S` 开头与 `-E`，能正常显示的值在筛选器落入无效零值组，组内顺序还受 Swift `Set` 逐进程哈希种子影响。F-059 无解析成功标志，失败一律留 0，与合法 `S00E00`、「无」共享零值。**跨端比对**：本轮直接翻本地 `~/code/MoviePilot-Frontend`（v2.15.6，`19cead06`）—— F-057 的正则与 TV 逐字相同、同样捕获第二季号却从不消费，属**同源同缺陷**，TV 单方面修改反而是与 Web 分叉；F-058 在 Web 不存在第二套语法（`TorrentCard.vue:177` 直显原始 `season_episode`），「对齐 Web」只能靠删掉卡片美化实现、是显示退化；F-059 最狠的「集号解析失败被冒充整季」是 TV 独有（Web 用 JS `parseInt`，超长数字算出巨大浮点数而非解析失败）。据此向用户给出「无『对齐一下』这个选项，只有修或不修」的判断，用户选择只修 F-059 的归类错误。 | 3 | F-057、F-058 登记为**用户决定跳过**（仅排序位置、筛选本身正确、上游是否产出这些格式从未验证；F-057 若真修应上游 Web 先行）。F-059 登记为**部分修复**：`ParsedSeason` 初始化器新增 `hasEpisodeMarker`，`isWholeSeason = !hasEpisodeMarker` —— 整季当且仅当压根没有 E 标记，畸形输入与「无」一样留在无效组，排序位置不变；无成功标志/零值多义性等排序侧影响随前两条一并跳过。新增 `ParsedSeasonTests` 9 例（全仓此前**无任何** `sortSeasonOptions` 测试）：2 条阳性断言（`isWholeSeason == false`；`sortSeasonOptions(["S01E05", "S01E<20位数字>"])` 中畸形项排在真实集号之后）+ 7 条阴性对照（`S01`/`S12` 仍为整季、正常集号与集范围排序不变、整季组仍整体靠前、「无」与空输入行为不变、单选项与空数组原样返回）。定向 9/9；反向验证把该行还原为 `isWholeSeason = true`：2 失败 / 7 通过，失败的正是两条阳性断言。全量套件 **929/929 通过、零失败**。计数勘误：`grep -c "^Test Case"` 得 928，与 920+9 不符；逐名比对两次运行的用例集合后定位到 `MediaDetailViewHeaderActionTests testDetailReadyHandlerKeepsRetryPendingWhenSubscriptionRefreshFails` 的输出行被 App 侧日志（`    "Loca...`）**前缀拼接**，行首锚点失配导致少计 1 条，改用非锚定模式与唯一名去重后确认为 929（该用例本身 passed 0.014s）。 |
| S465 | F-060 Release 日志治理登记 | 用户在听取「触发条件/影响」报告后（本轮报告结论：只影响挂着调试器的 Release 运行，真机 App Store 版 stdout 无出口，且无凭据泄漏证据）指示「都替换了，不要用 print 了」，即不做跳过而是全量治理。实测生产端直接 `print` 共 **67 处、散在 16 个文件**（审计原记 80 处/15 文件，差异为计数口径：本次用括号配平逐点提取，排除了 `hasSameMutationFingerprint(` 这类子串误命中，以及 `Logger.swift` 自身被 `#if DEBUG` 门控的那一个 `print`）。分级按原文语义落位：错误/失败路径 → `Logger.error`；Keychain 清理失败、选中规则失效 → `Logger.warning`；加载成功计数 → `Logger.info`；取消路径、`APIService` 的 `DEBUG:` 前缀与 CustomFilter 逐资源追踪 → `Logger.debug`；与级别重复的前导 emoji（❌/✅/⚠️/ℹ️/🔍）一并去掉。 | 67 | **配套隔离改动**：首次替换后编译失败 2 处 —— `HomeViewModel:129`、`MediaDetailViewModel:423` 位于 `nonisolated` 的 Task 闭包内，而本 target 设了 `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`，`Logger` 的静态方法默认是 MainActor 隔离。据此把 `Logger`/`LogHandler`/`PrintLogHandler`/`Logger.Level` 显式标注 `nonisolated`、`handler` 标注 `nonisolated(unsafe)`（`bootstrap` 全仓无调用方，`handler` 实际只有默认值一个写入点）。**守卫测试**：新增 `NoDirectPrintInProductionTests`，经 `#filePath` 上溯仓库根遍历 `MoviePilot-TV/**.swift`，用状态机挖空字符串字面量（含三引号多行串与 `#"..."#` 原始串）、`//` 行注释、`/* */` 块注释（支持嵌套）后按 `(?<![\w.])print\s*\(` 匹配，按文件名豁免 `Logger.swift`，定位失败时 `XCTSkip` 而非误报。**双向验证**：临时投放探针文件同时含真实调用、注释里的调用、字符串里的 `print(` 文本与 `fingerprintCheck(` 调用，守卫只报出唯一真实调用所在行（`__PrintGuardProbe.swift:5`）、其余三种一律不报；探针随即删除并确认残留计数为 0。全量套件 **930/930 通过、零失败**（929 + 新增守卫 1）。另补 **Release 配置构建验证**（`-configuration Release`，因为本次改动的核心正是 Release 行为）：`** BUILD SUCCEEDED **`，唯一 warning 为既有的 `MediaPreloader.swift:255 result of 'try?' is unused`，与本改动无关。影响面已记入 findings：`PrintLogHandler` 整体位于 `#if DEBUG` 内，替换后这 67 处在 Release 中完全不再执行（闭包也不求值），Release 侧零输出零留存，代价是 TestFlight/Release 也拿不到这些诊断；若日后需要 Release 可观测性应改 `LogHandler` 实现而非恢复 `print`。 |
| S466 | F-078 分享业务 ID 身份修复登记 | 用户在逐条过 P3 时听完触发链与影响面后决定修。本项身份链路：`SubscribeShare.id`（`Models.swift:2934` 起）既当 SwiftUI 行身份、又经 `generateUniqueKey`（`:1086`）当去重 key，被 `SearchViewModel.swift:820` 与 `ExploreViewModel.swift:1188` 的 `deduplicateSubscriptionShareMedia` 用来**过滤掉**已见项。原实现三处退化：`raw_id.map(String.init) ?? share.id` 只认 nil 故 `0` 会变成 `share:0`，令所有 0 号分享互撞而被静默吞掉；缺 ID 时身份用可变标题 `Share--<share_title>-<share_user>`，分享人改一次标题即换身份（焦点跳 + `pendingForkReceipt.shareID` 失配可重复建订阅）；三者全空时每次解码随机 UUID。**审计两处「剩余未验证」本轮查清**：① 分享 ID schema —— `~/code/MoviePilot app/schemas/subscribe.py:153-155` 为 `id: Optional[int] = None`，即**后端设计上允许缺 ID**，列表代理 `MoviePilotServerHelper._handle_list_response`（`app/helper/server.py:336-342`）200 时直接 `return res.json()` 逐条不校验，故缺 ID 属契约内输入；② Fork 语义 —— `app/api/endpoints/subscribe.py:715-733` 建订阅前 `sub_dict.pop("id")`，订阅内容全来自 name/year/type/keyword/媒体 ID/规则字段，ID 仅喂给成功后的 `async_sub_fork(share_id=sub.id)`（上报中心服务器「复用人次」）且**返回值不判**，`None` 时 URL 退化为 `/subscribe/fork/None`，故审计独立复核怀疑的「复用错目标」静态上不成立，真实后果仅为中心服务器统计归属可能记错、本机不可见。**用户两次提议均经复核后调整方向**：（a）用户提出「TV 也不该去重，跟 Web 对齐」—— 经 `Paginator.swift:242-243` 注释（「跳过少量只包含已知项目的页面」）与 `:283-284`（`if !hasNewContent && currentError == nil { hasMore = false }`）确认 TV 的 `hasMore` 收敛**依赖 processor 返回值**，去重即该值来源；移除后 `hasMore` 只能靠空页收敛，遇全已知项页面会无限追加重复卡且永远翻不到底（Web 能不去重是因为它没有 `hasMore`、终止在数据层，key 只喂虚拟滚动的行高缓存/ResizeObserver/行定位，无删数据权力），故驳回提议、保留去重；（b）审计原「最小方向」为「要求唯一正业务 ID；非法记录明确过滤/拒绝」，在缺 ID 属契约内输入的前提下等于剔除合法分享 —— 正是本项用户影响中的「丢项」本身，用户明确不要，故亦不采纳。**最终方向**：保留去重机制、只修 key 公式并与 Web 对齐 —— `generateUniqueKey` 把有效分享号收紧为**正**整数（Web 卡片 key 的 `item.id \|\| ...` 里 0 同为假值）；`SubscribeShare.id` 改为正 ID 时仅由 ID 决定（`Share-<id>`），缺 ID 时按 Web 兜底链 `media_id → tmdbid → doubanid → bangumiid → anilistid → name` 取首个非空媒体标识再拼 `share_user`（用内容固有属性取代可变的 `share_title`），全退化才退回随机 UUID 作 fail-open。 | 7 | 新增 `shareMediaIdentityComponent` 辅助；测试 7 条：0 号/负数不再互撞、缺 ID 落到媒体标识、标题变更后身份不变且仍跨页去重、正 ID 身份不受标题与分享人影响、退化输入不被去重吞掉、重复正 ID 现状固化。定向 `SubscriptionShareDedupTests` 10/10。**反向验证**：保留新用例、两处实现退回修复前 → 6 挂 / 4 过，4 条通过者即阴性对照（`testDuplicatePositiveRawIdsStillCollapseToOneRecord` 固化「重复正 ID 仍去重掉一条」，另 3 条既有正 ID 用例为回归对照；注意 `testShareWithoutAnyIdentityFieldIsNeverDeduplicatedAway` 在旧码下亦挂，因其测的正是「不再用 share_title 兜底」这条新行为，非阴性对照）。全量 **937/937 通过、零失败**（930 + 7），与上一轮日志逐名比对确认无用例消失。残留：重复**正** ID 仍会去重掉一条（保留去重的固有代价，Web 靠不去重规避，无唯一键可区分）；中心服务器实际 ID 分布与真机 tvOS 焦点表现未验证。 |
| S467 | F-101 SSE 事件组帧修复登记 | 用户在逐条过 P3 时听完触发链与影响面后决定修。**审计原「最小方向」（只在既有取行之上按空行组帧并合并 data）经复核后不成立**：`URLSession.AsyncBytes.lines` 即 `AsyncLineSequence` 会**丢弃空行**，而空行正是 SSE 事件结束标志，故事件边界在取行阶段就已丢失 —— 语义上完全不同的 `data: a\n\n data: b\n\n`（两个独立事件）与 `data: a\n data: b\n\n`（一个合法多行事件）经 `.lines` 之后同为 `["data: a", "data: b"]`，不可区分。已用本地真实 SSE 流复核（`Content-Type: text/event-stream`，三个单行事件经 `.lines` 只取出 3 行，分隔空行全部消失）。此事实的实践后果在实现中途即被实测证实：按审计方向在 `.lines` 之上补累积，非但修不好多行事件，反而把原本正确的**连续单行事件**合并成一个非法载荷（`Unexpected character '{' after top-level value around line 2`）而导致全线报错，一度令 7 个既有用例转挂（`BackendCompatibilityReadOnlyTests` 两条、`BackendCompatibilitySideEffectTests` 一条、`ResourceResultViewModelTests` 三条、`SearchViewModelTests` 一条）；这同时反证了「改动前逐行解码」对单行事件本无错，真实缺陷只在多行事件。**最终实现**：新增 `MoviePilot-TV/Services/SSEFramer.swift`（`nonisolated struct`，只做事件组帧，不解码 JSON、不判定业务语义），自己按字节切行 —— `consume(byte:)` 在 `0x0A` 处成行（UTF-8 安全：续字节均 ≥ 0x80，不会切坏字符）再交给 `consume(line:)` 做协议层累积（忽略 `event:`/`id:`/`retry:`/`:` 注释行，冒号后只吃一个空格），生产端 `APIService.streamSSE` 与兼容探针 `BackendCompatibilityTests.readSSEStream` 均改为 `for try await byte in ...` 并共用该规则。**与规范的唯一有意偏差**：流结束时若仍有未以空行收尾的挂起事件，`flush()` 交付而非按规范丢弃 —— 改动前是「data 行一到即处理」，断线前最后一个事件（可能是 `done`）一直收得到，严格丢弃会静默降低容错；`flush()` 亦先补齐停在半行上的残余。性能对照（1.3 MB / 401 事件，`-O`）：字节遍历 9.5 ms vs `.lines` 15.9 ms，无回归。 | 20 | 新增 `SSEFramer.swift` 与 `SSEFramerTests.swift`（协议层 12 条 + 字节层 8 条），生产解析器与兼容探针改为字节遍历。定向 `SSEFramerTests` 20/20；此前转挂的 4 个 SSE 相关测试类 96/96。**反向验证**：把两个入口逐字复刻改动前代码（`String(line.dropFirst(5)).trimmingCharacters(in: .whitespaces)`、`flush()` 恒返回 nil）并保留全部新用例 → 14 挂 / 6 过，6 条通过者即阴性对照（`testSingleLineEventContentIsUnchanged`、`testBlankLineWithoutPendingDataReturnsNil`、`testConsecutiveSingleLineEventsStaySeparateThroughByteStream`、`testMissingTrailingBlankLineStillYieldsSeparateEvents`、`testHeartbeatsAndCommentsProduceNoPayloads`、`testMultibyteCharactersSurviveByteSplitting`）；其中 `testConsecutiveSingleLineEventsStaySeparateThroughByteStream` 在旧码下亦通过，已在 findings 中如实说明，印证旧码对单行事件本无错、本项真实收益面只有合法多行事件。全量 **957/957 通过、零失败**（937 + 20），与 F-078 一轮的 937 逐名比对确认零用例消失。残留：当前后端 8 处 SSE 生产端（`app/api/endpoints/search.py:113-128` 的 `_sse_event`、`app/api/endpoints/system.py:788` 等）全部为 `f"data: {json.dumps(...)}\n\n"` 单一物理行形态，触发条件**不可达**，属前瞻性健壮性修复；heartbeat/comment 行、单事件最大尺寸、Content-Type 与明确终止保证仍未验证（注释行忽略已有对应用例覆盖）。 |
| S468 | F-122 识别 outcome 折叠残留闭合登记 | 用户在逐条过 P3 时听完触发链与影响面后决定修。本项此前已部分修复（`recognizeTmdbId` 改 `async throws -> Int?`，两段 error/cancel 均 throws；`MediaActionHandler` 空标题与网络错误不再弹「媒体不存在」），本轮闭合**最后一处残留折叠**：原实现把首段 `/media/search` 的错误存进 `firstStageError` 后继续走兜底 `/media/recognize`，但兜底「请求成功却给不出可用 ID」时会直接落到函数末尾的 `Logger.info("识别失败")` + `return nil`，**把暂存的首段错误静默丢弃**。调用方据此断言「确定无匹配」并弹出「未识别到此媒体的TMDB信息」，而实际上一次完整查询都没完成过 —— 典型触发为 Wi-Fi 切换 / DNS 抖动导致首段超时，兜底连通但后端认不出该片，用户被误导为「片子在 TMDB 上不存在」且不会重试。**修复实现（3 行）**：在末尾 `return nil` 之前加 `if let firstStageError { throw firstStageError }` —— 只要首段失败过就不允许把「查询没做完」伪装成「真无匹配」；抛出后由 `MediaActionHandler.swift:31-66` 既有的 `catch` 分支按「识别失败」处理（`Logger.error` + 返回 nil，**不弹**提示），真 no-match（两段均成功且均无结果）仍返回 nil 不受影响。**有意不改的一处**：兜底「识别出来但类型不符」仍 `return nil`，该分支基于一次**成功**的兜底查询，属合理否定判定，不属于「查询未完成」。 | 2 | 新增 2 条用例：`testSearchFailureThenFallbackNoMatchThrowsInsteadOfReturningNil`（首段 500 + 兜底空结果）、`testSearchFailureThenFallbackMissingIdThrowsInsteadOfReturningNil`（首段 500 + 兜底返回 0 号无效 ID）。定向 `TmdbRecognitionPositiveIDTests` 10/10。**反向验证**：撤掉这 3 行、保留全部用例 → 2 挂 / 7 过，7 条阴性对照中含 `testBothStagesNoMatchStillReturnsNil`（真无匹配仍返回 nil）与 `testSearchFailureThenFallbackSuccessReturnsId`（首段失败但兜底成功仍返回 ID），二者在改动前后必须不变。全量 **959/959 通过、零失败**（957 + 2），与 F-101 一轮的 957 逐名比对确认零用例消失。**剩余未决（产品意图，非缺陷）**：Home 的「搜索资源」按钮（`HomeView.swift:295`）在真 no-match 时仍走 `else` 分支用标题兜底导航，而 `getTMDBJumpTarget` 已置 `showTMDBNotFoundAlert = true`，全局弹窗（`ContentView.swift:105` → `MediaActionModifier`）随之弹出，即同一动作同时产生 alert 与 navigation；改动前该提示是**误导**的（网络错误也弹），现其内容已**属实**（「未识别到此媒体的TMDB信息」+ 已按标题搜索）。审计原文注明「Home 对真正无匹配是否仍需信息提示属于产品意图，不能由静态审计代定」，故本轮不擅自改动，留待用户裁决。 |
| S469 | F-122 无匹配提示按调用方区分登记 | 用户听完「为什么搜索资源会弹出未识别TMDB信息的窗」的解释后裁决「就按 A 改」—— 即识别照旧做，但这条**自愈**的降级路径不再弹窗。**背景**：Home 的「搜索资源」按钮（`HomeView.swift:295`）确实**依赖** TMDB 识别结果 —— `searchResourcesTarget` 用 `keyword: item.apiMediaId ?? ""` 构造请求，`apiMediaId` 取自 `identity.mediaKey`（形如 `tmdb:12345`），而 HomeView 现造的 `info` 只带 title/type/year，故必须先识别出 TMDB ID 才能构造正常搜索请求；识别失败则走 `else` 分支退回按标题搜索，**动作照样完成**。问题在于该按钮复用了为「TMDB详情页」写的 `getTMDBJumpTarget`，连带继承了 `showTMDBNotFoundAlert = true` 这一副作用，全局弹窗（`ContentView.swift:105` → `MediaActionModifier`）随导航一起弹出 —— 同一动作**同时**产生 alert 与 navigation，用户被一个必须点「确定」的模态框拦在一个已经自愈完成的动作上。弹窗描述的事实本身无误（确实没认出来），问题在于它拦得没有道理。**修复实现**：`getTMDBJumpTarget` 增加 `notifyWhenUnrecognized: Bool = true` 形参，仅在 `true` 时置 alert；Home「搜索资源」传 `false`。**默认值保持 `true`**，另三个「TMDB详情页」入口（`HomeView.swift:277`、`MediaContextMenu.swift:47`、`MediaDetailView.swift:908`）行为不变 —— 用户以 TMDB 为目标时找不到就该告知。 | 5 | 新增 `TMDBJumpAlertPolicyTests` 3 条（默认弹 / 传 false 不弹 / 已有 ID 不弹）与 `TMDBJumpAlertCallSiteTests` 2 条（源码守卫：Home「搜索资源」传 `false` 且「TMDB详情页」保持默认；`MediaContextMenu` 与 `MediaDetailView` 不得出现该参数）。定向 5/5。**反向验证**：同时撤掉形参开关与调用点传参、保留全部用例 → 2 挂 / 3 过，3 条阴性对照为 `testUnrecognizedMediaNotifiesByDefault`、`testResolvedIdNeverNotifies`、`testOtherTMDBJumpEntryPointsKeepDefaultNotification`。行为用例走「标题经季数清洗后为空 → 不发网络请求」的早退路径（`title: "第二季"`），以规避 `APIService.shared` 在 init 时复制 `URLSessionConfiguration`、测试桩只对隔离实例生效这一限制。全量 **964/964 通过、零失败**（959 + 5），与上一轮 959 逐名比对确认零用例消失。F-122 至此**全部闭合**（错误折叠 + 误弹窗）。 |
| S470 | F-135 option 身份规范化修复登记 | 用户在逐条过 P3 时听完触发链与影响面、并在追问「Web 是不是也有这个问题」后，明确要求「不要光照抄，有些是TV端做的好的地方」，据此按审计原「最小修改方向」落地四处改动。**①插件筛选项去重**：`PluginFilterOption.id` 直接取 `value`（`ExploreViewModel.swift:53`），`ExploreView.swift:475` 又用同一个 `value` 作 Picker 的 tag —— 一个 value 同时充当身份与取值，插件若在两个标签声明同一个 value 就会同时产生重复 ID 与重复 tag，`ForEach` 的 diff 与焦点随之不稳。实现为把原 `collectOptions` 拆成「按声明顺序展开」+「入口处按 `JSONValue` first-wins 消重」两层：去重只做一遍，故 `content` 与 `items` 之间的**跨层级重复**也被同一遍扫描消掉（父级 `items` 与子级 `content` 声明同一个 value 时先出现的 content 胜）。`rangeOptions` 未动 —— 其值单调递增且已有相邻消重，本就唯一。**②③目录规范化**：`AddDownloadViewModel.targetDirectories` 与 `SubscribeSheetViewModel.savePathOptions` 原本是两套不一致的写法（前者 `guard let path = item.download_path` 只挡 `nil`；后者先去重、从不 trim），改为同一套顺序：**先 trim、再丢空、最后去重**。顺序是本质 —— 反过来的话纯空白路径会活下来变成选项。修完后内建「自动」选项（value 为空串）在列表中天然唯一，即审计要求的「保留唯一自动项」，**无需任何额外占位代码**，也遵守审计明令的「**不新增 option ID 层**」。**④顺带加固（用户裁决「并入一起做」）**：`TransferDirectoryConf.storage` 由 `let storage: String` 改为 `String?`。查证结论是**机制成立、可达性未证实**：后端 `system/setting/public/{key}` 对 Directories 走 `SystemConfigOper().get(...)`，返回**未经 pydantic 校验的原始配置**（`app/api/endpoints/system.py:807`），写入端 `set_setting` 也只做 `list(filter(None, value))` 而不校验字段（`:911-914`），而后端 schema 本就是 `storage: Optional[str] = None`（`app/schemas/system.py:145`）；TV 侧 `decodeOrUnwrap` 用的是裸 `JSONDecoder()`（`APIService.swift:416-422`），非可选字段一旦缺失即抛 `keyNotFound`，令**整个目录数组**解码失败，`AddDownloadViewModel` 的 catch 随即置 `loadErrorMessage`，`AddDownloadSheet.swift:145` 禁用「确认」且重试永不成功 —— **概率低但中招即全瘫**。但 Web 目录设置页 `addDirectory()` 恒写 `storage: 'local'`（`AccountSettingDirectory.vue:207`），后端既无播种默认值、也无补 `storage` 的迁移代码，故未找到会产生该形态配置的路径；且 Web 自身类型声明同样是 `storage: string`（`api/types.ts:1482`），其 runtime 的 `undefined/null` 兜底已超出自身类型，可见并非「TV 比 Web 少防一层」，故**不单列新发现**、仅作防御性加固并入本项。**跨端复核结论：四处改动无一为照抄 Web** —— Web 的下载弹窗与字幕弹窗的 `convertToUri` **不做 trim**，三个弹窗里只有 `SubscribeEditDialog.vue:350` 做了，纯空白路径在 Web 侧同样会变成空选项，故②③比 Web 现状**更完整**；Web **没有**内建「自动」显式选项（用 `:placeholder` 代替），TV 的是遥控器可聚焦项、属有意设计，审计「保留唯一自动项」即明确保留它，不得照抄删除；③插件一侧 Web **无对应层** —— `ExtraSourceView.vue:67` 把 `filter_ui` 交给 `FormRender.vue` 用 `h()` 原样透传给 Vuetify 组件，外层 `:key="index"` 按下标而非 value，不存在「value 兼作身份」的中间层，审计的「不新增 option ID 层」正是明确否决照搬 Web 那种身份/取值分离的模型。 | 13 | 新增 13 条用例：`DirectoryOptionNormalizationTests` 9 条（空串 / 纯空白 / 远程空串一律不进列表、组合后的 Picker ID 唯一、trim 先于去重、同后缀跨存储保持区分、声明顺序保持、订阅侧同一套规范化、两条 `storage` 缺失/为 null 仍能解出整个数组）与 `DynamicSourceBehaviorTests` 4 条（同级重复 first-wins、跨 `content`/`items` 层级重复 first-wins、无重复时顺序不变、去重按 `JSONValue` 的 case 区分而非字符串比较）。定向 39/39。**反向验证**：把四处生产改动一并还原为改动前原文、并仅把测试 helper 的 `storage` 形参改回非可选以恢复编译 → **8 挂 / 5 条阴性对照通过**；失败输出即真缺陷本身（`["", "   ", "qbittorrent: \n ", "/downloads"]`、组合后 `["自动", "", "qbittorrent:  ", "/downloads"]` 出现两个空 ID、`DecodingError.keyNotFound: Key 'storage' not found`、`DecodingError.valueNotFound: found null`、`["   ", " /downloads ", "/downloads", "/media"]`），5 条对照为 `testDeclarationOrderIsPreserved`、`testLocalAndRemotePathsWithSameSuffixStayDistinct`、`testSubscribeSavePathOptionsDropsNilPaths`、`testPluginFilterDistinctOptionsKeepDeclarationOrder`、`testPluginFilterDeduplicationRespectsJSONValueIdentity`。全量 **977/977 通过、零失败**（964 + 13），计数与新增条数吻合。 残留：`storage` 缺失的可达性仍未证实（仅记录为防御性加固，未单列发现）；③ 的真实触发前提是「插件确实吐了重复 value」，该频率仍未验证，属 TV 独有前瞻性防线；Web 侧下载/字幕弹窗不做 trim 的同类缺陷未在本次范围内处理（只读参考仓库，非本项目改动对象）。 |
| S471 | F-140/F-141 搜索提交词规范化与年份词法修复登记 | 用户在逐条过 P3 时听完两条的触发链与影响面后裁决「一起修」。两条同处 `SearchViewModel` 的搜索链路，且审计的「最小修改方向」本身就把 F-141 定义成「在 F-140 的同一规范化 query 上复用目标后端等价的年份边界」，故一轮落地。**F-140**：`autoSearch()` 原先 `guard !query.isEmpty` 后 `let searchQuery = query` 直接取用户原串，该串随即成为 `submittedQuery`（`:433`）—— 而 `submittedQuery` 是**唯一写入口**，评分（`:168-200`）、分页器（`:597`）、请求（`:465`/`:549`）全读它。后端 `StringUtils.get_keyword`（`app/utils/string.py:634`）自带 `.strip()`，所以**发出去的请求本来就是干净的**，缺陷只存在于 TV 自己的本地评分层：`fuzzyMatchScore("Hamilton", "Hamilton ")` 走顺序匹配档、文本先被消耗完而返回 `-1`，精确标题反被 `Hamilton Musical`（包含匹配，实测 684）反超；若该精确项又无海报且热度 < 1，会被 `hasNoPoster && maxS < 50 && pop < 1` **整条淘汰**。纯空白（`"   ".isEmpty == false`）还能绕过旧守卫跑完整个聚合搜索。修复即审计原文的「只做一次 `.whitespacesAndNewlines` 规范化并让后端请求与本地评分共用」：`let searchQuery = query.trimmingCharacters(in: .whitespacesAndNewlines)` + `guard !searchQuery.isEmpty else { return }`。**刻意不写回 `query`**（搜索框保留用户输入原样，提交后就地改写更突兀），**也刻意只去首尾、不压缩内部空白**（`testInternalWhitespaceIsDeliberatelyPreserved` 固定这一非目标）—— 内部空白的匹配质量属评分层分档问题，动它会改变既有 `hasPrefix`/`contains` 分档的输入。**F-141**：年份词法由裸 `(19\|20)\d{2}`（扫首个四位串）改为 `[\s(]+((?:19\|20)\d{2})[\s)]*`，与后端 `get_keyword` 的 `[\s(]+(\d{4})[\s)]*` 同构 —— 四位数字必须**紧跟分隔符**才算年份。两处效应一并收敛：①数字片名不再被误当年份（`1917 2019` 现正确取 year=2019、回退词 `1917`，精确项由 `-1` 变 `1000`）；②剥年份改用 `replacingCharacters(in: match.range, …)`，**连前导分隔符与尾随右括号一起删**（`1917 (2019)` 由残留 `(2019)` 空壳改为干净还原成 `1917`）。**两处刻意收敛已注明**：①保留 `(19\|20)` 前缀而不照搬后端的裸 `\d{4}` —— 年份在 TV 侧用于给候选补「标题+年份」变体并放宽回退，把 `1234` 当成年份只会引入新误判，收紧是安全方向（比后端少识别只会失去加分，不会误加分）；②「剥完只剩空串 → `queryWithoutYear` 取 nil」**不改变任何可观测行为**（空串在 `fuzzyMatchScore` 里恒为 `-1`，进 `max` 必被原串分值盖掉），纯属契约收敛，故**未为其编造用例**。**跨端复核：两条都无 Web 可照抄** —— Web（`browse.vue`）把 `route.query.title` 原样交给 `MediaCardListView` 直接渲染后端列表，**根本没有本地评分层与本地年份层**，「最佳匹配」是 TV 独有面；F-141 更是把 TV 词法**收回后端既有契约**，而非对齐 Web。 | 5 | 新增 5 条用例（`SearchViewModelTests` 新扩展 `F-140 / F-141 搜索提交词的规范化与年份词法` 4 条 + 1 条阴性对照）：尾随空格规范化后请求与评分共用同一串且精确标题排第一、纯空白不发任何请求（媒体搜索与订阅分享请求数均为 0）、内部空白刻意保留、`1917 2019` 的数字片名不被当年份、`流浪地球 (2019)` 剥年份连括号一起删。定向 `SearchViewModelTests` 42/42。**反向验证**：把两处生产改动一并还原为改动前原文、保留全部用例 → **8 挂 / 1 条阴性对照通过**，失败输出即真缺陷本身（`submittedQuery` 为 `"Hamilton "`、请求以带空格串发出致 stub 只回默认项 `["Old Result"]`、`hasSearched` 为 true 且媒体搜索发出 4 次请求、`"1917 2019"` 的精确项被整条淘汰得 `[]`、`"流浪地球 (2019)"` 只剩扩展项 `["流浪地球特辑"]`），唯一的阴性对照为 `testInternalWhitespaceIsDeliberatelyPreserved`。**过程中修正的一处用例缺陷**：首版用 `waitForRequest(path:query:)` 断言请求串，但该 helper **无超时**，反向验证时因请求串始终不匹配而**死等、挂住整个测试进程**（实测 600 s 超时未返回）—— 已改为依赖「stub 按 `title` 值取响应」这一性质间接断言（请求串错则取不到数据、标题断言连带失败），既保留了对「请求规范化」与「评分用同一个串」的一半覆盖，又不再有挂起风险。全量 **982/982 通过、零失败**（977 + 5）。 残留：审计复算的 extended 分值 `484` 与实测 `684` 不符（`700 - 标题长度`，16 字符恰为 684），已如实记录、不影响结论方向；`(19\|20)` 之外的四位年份（如 `1899`）TV 不识别而后端识别，属有意收紧、只失去加分不误加；真实用户输入「尾随空格 / 数字片名」的频率仍未验证。 |
| S472 | F-050/F-056 Hero 主演取样顺序修复登记 | 用户在逐条过 P3 时听完触发链与影响面后裁决「直接修吧」。本项在 G07 第三裁中已把 F-056（无名氏白占名额）并入，故一轮落地**两条同源口径**，顺序即裁决原文：`processActors` → 过滤 trim 后空名 → `prefix(4)`。**①顺序颠倒**：原实现是 `heroTopActors = StaffManager.processActors(persons: Array((fullDetail.actors ?? []).prefix(4)))` —— 先截断、后按 `person.id` 合并，于是前四条里的多角色重复（TMDB cast 允许同一个人以不同 `character` 出现多条）被折叠成一个，Hero 只剩 2~3 人。后端 `actors` 取自 `credits.cast`（`app/core/context.py:481-486`），只按 `known_for_department == "Acting"` 过滤而**不去重**，所以重复是结构性可能的。而尾部「Hero 完全为空才用分页数据补」的分支只在**彻底为空**时兜底，半空状态永远补不上 —— 这是缺陷从「少一个人」变成「长期少人」的关键。颠倒后与分页那一路同构（`actorsPaginator` 的 processor 本就是 `mergeActors`），故该兜底分支里的 `prefix(4)` 是对的、无需改动。**②空名过滤**：视图侧 `MediaDetailView.swift:1037` 是 `topActors.compactMap { $0.name }.joined(...)`，name 为 nil 的人渲染出来只是一个空档，却照样消耗四个名额之一。判据是「trim 后为空」而非「非 nil」，故纯空白名同处理；过滤只**判空**、**不改写**姓名（带空白的真名原样保留，视图直接拼 `$0.name`，改写属于另一处口径）。**③刻意收敛**：过滤只放在 Hero 取样这一层，**不下沉到 `mergeActors`** —— 演员货架要不要收无名者是另一条口径，verify_s006 已明确货架不受本项影响；尾部「完全为空才补」分支也保持原样，它只在 `heroTopActors` 彻底为空时兜底，此时过滤与否都不改变「这份数据本就没有可显示的主演」这一结论，为它加过滤只会扩大改动面。**跨端复核：无可照抄面** —— Web 根本没有 Hero 主演行，演员经 `/credits/...` 页面展示，与 TV 的 Hero 文本行形态不同。**过程中修正的两处**：(a) 首版把 `testMergedDuplicateKeepsAllCharacters` 放在阳性区，反向验证暴露它**新旧实现下同过** —— 旧实现取的前四条里重复项本就都在，合并结果同样是「角色一/角色二」，合并口径不随选取顺序变；它拦的是「用 Set 按 id 只留首条」这类**错误修法**而非本 bug，已改标为阴性对照并注明，未删除（它守的是另一种错法）。(b) 注释里 `:250-251`、`:251` 两处行号引用在本轮增行后已失效，且第一段「不丢信息」会被误读为覆盖过滤那一半，一并改为描述性引用并收紧表述。 | 9 | 新增 `MoviePilot-TV-Tests/HeroTopActorsOrderingTests.swift` 9 条（2 条阳性 + 7 条阴性对照）：重复项挤人后第四人得上位、无名氏不占名额、纯空白名同处理、角色合并保留（阴性对照）、无重复时取人不变、服务端顺序保持、不足四人不补足、空进空出、只判空不改写姓名。定向 9/9。**反向验证分两次单独还原**：①退回「先 `prefix(4)` 再去重」→ 当时那版 6 条用例中 **1 挂 / 5 过**，失败输出即真缺陷本身（`["甲", "乙", "丙"]` 而非 `["甲", "乙", "丙", "丁"]`）；②仅去掉空名过滤 → 9 条中 **2 挂 / 7 过**（`["乙", "丙", "丁"]`、`["   ", "乙", "丙", "丁"]`，均少掉第五人）。全量 **991/991 通过、零失败**（982 + 9），计数与新增条数吻合。 残留：真实重复分布与无名人物分布均未验证（未按可选项加临时真机日志统计频率）；F-056「后续完整结果可补位」那半本次未改动，触发条件仍是 Hero 彻底为空。 |
| S473 | F-169 货架持久选择的无障碍语义登记 | 用户在逐条过 P3 时听完「焦点与持久选择是两种状态」的解释后裁决「加吧」。**问题**：`ShelfChip` 的 `isSelected` 只驱动一处视觉 overlay —— 未选中且未聚焦的 chip 压一层 `black.opacity(0.2)`；Button 本身只提供名称与动作语义，没有任何 `.isSelected` trait 或等价 value。而**焦点与持久选择是两种状态**：`isFocused` 是「遥控器现在停在这个 chip 上」，`isSelected` 是「下方结果正由这个货架驱动」。用户把焦点移到别的 chip 但还没按下去时，视觉高亮仍在原货架、结果也仍归原货架，但 VoiceOver 对这个区别完全无从表达 —— 只能逐个听到 chip 名称。**修复**：新增 `accessibilityTraits` 属性（`isSelected ? .isSelected : []`）并 `.accessibilityAddTraits(...)` 挂在 Button 链尾。严格按审计裁决「在现有 Button 上一行条件添加 `.isSelected` trait」，**不加自定义 label/value**（名称已由 `Text(title)` 提供，另起一套文案会与可见文本分叉），**不引入 selection/focus 框架**（焦点仍由 `@FocusState` 管理）。两处实现取舍已注明：**①抽成属性而非内联三元** —— SwiftUI 的 trait 无法在 XCTest 里从渲染树读回，抽成属性后「只有选中项才带、焦点不参与」这条**规则**才能被直接断言；**②挂在链尾而非 `buttonBorderShape` 之后** —— `overlay` 会再包一层，放链尾确保 trait 落在最终的可访问元素上。**跨端复核：有可对照项但不是照抄** —— Web `MediaRecommend.vue:369` 确有 `:aria-current="activeIndex === index ? 'true' : undefined"`，但 Web 是鼠标/触摸的 hover 模型，焦点与选中在交互上重合；TV 是遥控器焦点模型，两者可分离，故只补这一层语义，不搬 Web 那套选中状态机。 | 6 | 新增 `MoviePilot-TV-Tests/ShelfChipAccessibilityTests.swift` 6 条，**刻意分两层**：4 条行为测试断言规则（选中项带 `.isSelected`、未选中项不带、**焦点落在未选中 chip 上不得播报已选中**、只加这一个 trait 不夹带其它），2 条接线守卫断言它真的挂在 Button 上且未引入 label/value/element。定向 6/6。**反向验证分两次**：①把属性置空、保留接线 → **2 挂 / 4 过**，两条阳性失败（`AccessibilityTraits(rawValue: 0)` 对 `rawValue: 4`）而两条阴性对照通过，接线守卫仍过；②只拆接线、保留属性 → **1 挂 / 5 过**，4 条行为测试全过、仅接线守卫失败。两次反向验证互为补充，证明两层各守一种失败模式（规则写对 vs 规则挂上），任一层单独存在都会漏掉另一种。全量 **997/997 通过、零失败**（991 + 6），计数与新增条数吻合。 残留：VoiceOver 实际播报措辞与真实困惑频率需真机验收，本轮未做（单测无法覆盖播报效果）。**后续处置**：用户就这条残留裁决「算了，这条要么就跳过吧」—— 不做真机 VoiceOver 验收。本项以 `已修复` 结项，**残留如实保留、不改记为已验证**；真机播报效果自此不再作为待办。 |
| S474 | F-078 分享兜底身份回归修正登记（外部 AI 审查 P2） | 外部 AI 审查报「分享兜底身份会碰撞，导致列表静默漏卡」，逐条回代码核对后确认**成立且系上一轮 `a92bf54` 引入的回归**。三组反例：`tmdbid: 0` 被 `map(String.init)` 变成非空 `"0"` 挡住后面的 `doubanid`（同段两行之上的 `raw_id > 0` 与 `MediaIdentifier.isValidManualMediaId` 本就是同一口径）；兜底公式不含 `media_source`，跨来源同号（两站点各自从 1 编号）撞成一条；不含 `season`，同剧不同季撞成一条 —— 后两类基线靠 `share_title` 区分，上一轮为遵守「不以可变字段冒充身份」删掉它时**未补上更稳定的区分字段**。后果是 `SubscribeShare.id` → `toMediaInfo()` → `generateUniqueKey` 拼出 `"share:<id>"` → `deduplicateSubscriptionShareMedia` 返回 false → **卡片静默不进列表**。另记方向性错误：Web 的 `get-item-key` 是 `ProgressiveCardGrid` 的**渲染 key**（`SubscribeShareView.vue:290-293`；`:183` 是直接 append、不做 key 过滤），且靠 JS 里 `0` 是假值自然穿透，移植到 Swift 的 `map(String.init)` 后该性质不成立 —— 把渲染 key 当去重身份是口径错配。修正：新增 `shareFallbackIdentity`，媒体标识先按有效性筛（数字 `> 0`；文本 trim 后非空且非 `"0"`/`"-1"`），再补回 `media_source`/`type`/`season` 与 `share_uid`（优先于可改名的 `share_user`）/`subscribe_id`，「谁分享的、从哪条订阅分享的」全缺时返回 nil 退回 UUID（字段不足以证明是同一条记录时不静默合并）；`share_title` 仍不参与。 | 10 | 新增 10 条回归（含审查点名的「无效数字 ID / 跨来源同号 / 同剧不同季」三组）与 3 条阴性对照。反向验证把 `Models.swift` 退回 `HEAD` → 9 条阳性全挂（含 `count: 1 vs 2` 的丢卡直接证据），4 条阴性对照（同记录跨页仍去重、改 `share_title` 不变身份、正业务 ID 身份不变、全退化必成新项）全通过，证明补字段没把身份拆得过散；恢复后定向 20/20。 |
| S475 | F-101 SSE 行尾与 BOM 缺口修正登记（外部 AI 审查 P3） | 外部 AI 审查报 `SSEFramer` 缺 CR-only 与流开头 BOM 处理，核对后均成立：只认 `0x0A` 时纯 `CR` 流被攒成一行、`flush()` 只剥一个尾部 `\r`，整条流拿到畸形载荷而报错（不是少一个事件）；流开头 BOM 让第一行不满足 `hasPrefix("data:")`，第一个事件的数据被整条丢弃。修正：`consume(byte:)` 拆为 BOM 前瞻层（仅流开头按 `EF BB BF` 三字节识别一次，不匹配则把已吃进的前缀原样补回）与 `consumeBody(byte:)`（`0x0D` 亦作行尾，用 `lastByteWasCR` 把 CRLF 的 `LF` 归入同一行尾）；`consume(line:)` 原有的尾部 `\r` 剥离保留，直接调用该入口的既有用例不受影响。 | 8 | 新增 8 条；反向验证把 `SSEFramer.swift` 退回 `HEAD` → 5 条阳性全挂（CR-only 2 处断言、BOM 三组），阴性对照全过。其中 `testCRLFIsOneLineEndingNotTwo` 经反向验证**修复前也通过**（旧 `consume(line:)` 本就剥行尾 `\r`），已按本仓库口径改标为阴性对照并写明所守为「新字节层 CR 处理不得把 CRLF 拆成两次断行」，不作为阳性证据。恢复后 SSEFramerTests 29/29；全量 **1016/1016 通过、零失败**。 |
| S476 | F-078 分享兜底身份编码加固登记（二轮外部 AI 审查 P2） | 同一轮第二份外部审查继续报 F-078：上一轮 `a92bf54` 的修正补回了字段，但**编码方式**仍是「用 `-` 把可变数量的字段拼起来」，不可逆 —— 不同字段组合可拼出同一字符串，碰撞只是从「漏字段」换成了「分隔符歧义」。逐组构造后确认**成立**，四组反例：①站点原生 ID 对 TMDB ID（`media_id=9001` 与 `tmdbid=9001` 落同一槽）；②字段内容冒充分隔符（`media_source=mteam` + `media_id=42` 对 `media_id="mteam-42"`）；③季号冒充用户名前缀（`season=1` + `share_user=alice` 对 `share_user="s1-alice"`）；④同名槽位（`share_uid` 与 `share_user` 恰好同值，无法区分来源）。修正为**定长带标签槽位 + 长度前缀**：媒体标识仍只取第一个有效项（保留「分享人改名不改身份」这条原始口径），但**带上其字段名**（`media_id:` / `tmdbid:` / `doubanid:` / `bangumiid:` / `anilistid:` / `name:`）；`share_uid` 与 `share_user` 以 `u:` / `n:` 区分来源；每槽统一编码为 `n`（缺省）或 `s<UTF-8 字节数>:<值>`，值内出现 `-`、`\|` 或数字均不再有歧义。**刻意未采纳**「每个 ID 字段各占一槽」的写法 —— `name`（订阅名称）是可改字段，让它无条件参槽会直接推翻上一轮「改名不改身份」的修正，故仍保留「第一个有效项」的选择语义、只给中选值打上字段名标签。测试侧把原先断言整串字面量的用例改为断言**槽位形状**，便于日后调整槽序。 | 6 | 新增 4 条碰撞回归（对应上述四组反例）+ 2 条结构断言改写。反向验证把 `Models.swift` 退回上一轮的 `-` 拼接版本 → **4 条碰撞用例 + 2 条结构断言全挂**，确认四组反例都是可复现的构造而非理论推演。恢复后 `SubscriptionShareDedupTests` 24/24；全量 **1024/1024 通过、零失败**。 |
| S477 | F-122 类型不符残留折叠闭合登记（二轮外部 AI 审查 P2） | 二轮外部审查指出 F-122 还有**第三条**漏网路径：`APIService.swift:2239` 的「兜底识别出的类型不符」分支是直接 `return nil`，**走不到**方法末尾那个 `throw firstStageError`。核对成立 —— 首段 `/media/search` 500/超时、兜底 `/media/recognize` 返回 200 但认成另一类型（期望「电影」、兜底给「电视剧」）时仍然返回 `nil`，`getTMDBJumpTarget` 继续把它当「媒体不存在」并弹误导提示，而首段其实从没查完。修法与 2026-09-12 那处同源，在该分支补同一口径：`if let firstStageError { throw firstStageError }` 后再 `return nil`。**边界由阴性对照守住**：两段都成功、首段确实查完且无匹配、兜底明确认成另一类型时，`nil`（= 不是这部媒体）仍然正确 —— 防止「类型不符就无条件 throw」的过度修正。 | 2 | 新增 2 条（1 阳性 + 1 阴性对照）。反向验证还原该分支 → **阳性 1 挂**，阴性对照与既有 2 条 F-122 用例（兜底 no-match、兜底缺 ID）全部通过。恢复后 `TmdbRecognitionPositiveIDTests` 11/11；全量 **1024/1024 通过、零失败**。 |
| S478 | F-140 测试诚实性修正与残留登记（二轮外部 AI 审查 P3，用户裁决「行为不动」） | 二轮外部审查指出 `testWhitespaceOnlyQueryDoesNotStartSearch` 只用**全新** ViewModel，而它断言的 `hasSearched == false`、`isLoading == false`、`bestResults.isEmpty`、`submittedQuery == ""` 本来就都是初始值，无论实现有没有清过状态都成立 —— **钉不住**「上一轮结果仍留在屏幕上」和「在途请求不被取消」这两条它看起来在守的语义。核对成立。用户裁决「行为不动，只修测试 + 记残留」。处置：①旧用例只保留真正有判别力的部分（纯空白不发请求、不记提交），删去无判别力的状态断言；②新增 `testWhitespaceOnlySubmitKeepsPreviousResultsAndStaysNoop`，从「已搜出结果」起步，固化纯空白提交为**完全 no-op**（不清旧结果、不改 `submittedQuery`、不抬 `isLoading`、不追加请求）；③新增 `testWhitespaceOnlySubmitDoesNotCancelInFlightSearch`，**走 `.resource` 而非 `.unified`** —— 只有资源分支把在途任务存进 `searchStreamTask`，`.unified` 用的是几个局部 Task，拿 `.unified` 写这条会得到一个测不出东西的假绿。**残留（刻意保留，记录备查）**：纯空白提交不取消在途搜索、不清旧结果；取舍是「敲几个空格不该被当成一次搜索，更不该把屏幕清空」。将来若要改成取消，**必须连带处理 `isLoading`** —— `finishSearchIfCurrent` 是这条路径上唯一清 `isLoading` 的出口且带 generation 守卫，随手 `cancel()` + 递增 generation 会让在途任务的 `defer` 因 generation 不匹配直接返回，`isLoading` 永久卡在 true。 | 3 | 旧用例改写 1 + 新增 2（均属**现状固化/回归钉**，本轮**无生产代码改动**，`SearchViewModel.swift` 全仓 diff 为空，故不适用反向验证）。判别力另法验证：在实现中临时插入审查建议的 `searchStreamTask?.cancel()` 后再 `return` → `.resource` 用例转挂（在途搜索被取消、结果永不发布），另外两条仍绿，证明它有判别力且不误伤。过程中自查修掉两处**测试自身**的错误：一是先写成 `.unified` 版本（插入上述 cancel 后照样全绿，属假绿）；二是曾 `await` 包装 `autoSearch()` 的外层任务，而资源分支在 `autoSearch()` 返回后才真正开跑流，等于什么都没等 —— 改为轮询状态，并把上限放宽到 5s 以容纳实现里 `done` 后 1.5s 的收尾等待。恢复后 `SearchViewModelTests` 44/44；全量 **1024/1024 通过、零失败**。 |
| S479 | F-052 失实注释勘误登记（二轮外部 AI 审查 P3） | 二轮外部审查报 `StaffManager.swift:229`「改完代码没改注释」，核对**成立**：注释原文「优先级：角色名 > 原始职位 > 角色列表 > 兜底『职员』」描述了一个**不存在的 `staff.job` 分支** —— 能走到该 fallback 恰恰说明没有任何人产出过 canonical 职位分组，`staff.job` 在此必然解析不出 key，显示标签实际只由 `character` → `roles` → 「职员」决定。已按实际代码改写注释，并补上「`roles` 是逐项规范化后再拼接，故 `["", ""]` 不再生成孤立 `/`」这一说明。**纯注释修正，零行为变更。** 同批审查提出的其余两条（`MediaDetailViewModel.swift:191` 超长叙事注释、`SearchViewModel` 空白分支）分属风格建议与已裁决项，本轮不改。 | 0 | 无新增用例（纯注释）。全量 **1024/1024 通过、零失败**。 |
| S480 | F-078 三轮审查：哨兵误用回归、身份门槛收紧与失效断言登记（外部 AI 审查 P2/P2/P3） | 三轮外部审查又提三条，**全部成立**，其中两条是**上一轮我自己引入的**。①[P2] `share_uid`/`share_user` 被套了 `normalizedTextIdentifier` 的数值哨兵规则，而这两个是「谁分享的」（`share_user` 是显示名，后端 `Optional[str]` 从未规定叫 `"0"` 的名字无效）。两条记录只差分享人（`"0"` 与 `"-1"`）、其余字段相同且都带 `subscribe_id` 时，两个名字都被清洗成 nil → 拼出同一 key → 后者静默过滤，又是漏卡。**须如实记录经过**：二轮我本已写成 `normalizedText`，收尾复查时以「保持既有口径、缩小 diff」为由**主动改回了 `normalizedTextIdentifier`** —— 那条回归正是这一步造出来的，我的「保守」改错了方向（哨兵规则是给后端用数字表示「没有」的**数值 ID 字段**定的，不该外推到「谁分享的」）。已改回 `normalizedText`。②[P2] 判定门槛仍是 `guard owner != nil \|\| subscribe != nil`，即「只要知道分享人就算数」；但同一人把同一部剧的 2160p 与 1080p 各开一条订阅分别分享很常见，缺 `subscribe_id` 时会被并成一条；只剩 `name` 兜底时同名电影 1984 与 2021 也会并。**关键认识**：长度前缀解决「怎么编码」，解决不了「凭什么认定是同一条记录」—— 编码再严谨也不能让本不唯一的信息变得唯一。**用户裁决：严格要求 `subscribe_id`**（`guard subscribe != nil`）。代价（缺 `subscribe_id` 时跨页不再去重、可见重复卡）按裁决接受，并用 `testRecordsWithoutSubscribeIdAreKnowinglyNotDeduplicatedAcrossPages` **显式固化**，防止日后被「顺手合掉」。③[P3] `testNumericSentinelStringsAreNotUsedAsMediaIdentity` 断言的 `!id.contains("Share-")` 在确定性 key 改成 `Share\|…` 后早已失效。实测结论**比审查更严重**：把 `normalizedTextIdentifier` 的哨兵判据**整条删掉**，该用例与相邻两条「哨兵」用例**依然全绿**（后两条守的是 `normalizedNumberIdentifier`，本就不覆盖文本字段）—— 即文本哨兵规则此前**零覆盖**。已改为断言身份是 UUID + 同一退化输入复解析必得不同身份，并新增真正有判别力的 `testTextSentinelMediaIdYieldsToValidDoubanId`。 | 5 | 新增 5 条（2 条反例 + 1 条判别力对照 + 1 条代价固化 + 1 条改写旧断言）。**三轮反向验证逐项做**：删 `normalizedTextIdentifier` 的哨兵判据 → 2 条转挂（原先是三条全绿）；退回旧 `guard owner != nil \|\| subscribe != nil` → 3 条转挂（两条反例 + 代价固化）。定向 `SubscriptionShareDedupTests` 29/29；全量 **1029/1029 通过、零失败**。 |
| S481 | SSE 逐字节读落在 MainActor 的性能回归 + 分享身份跨实例误合并（四轮外部 AI 审查 P1/P2） | 四轮外部审查（范围：本分支 21 个提交 / 51 个文件）报两条需修，**两条均成立，且都出在我前几轮自己的改动上**。①[P1] `10e0699`（F-101）把 SSE 读取从 `result.lines` 改成 `for try await byte in result`，逐字节的 `await` 落在 **MainActor**：约 530 KiB / 512 事件的流从 37 ms 劣化到 **11.09 s**。**根因不是「字节比行慢」而是每个字节一次 actor 往返** —— `.lines` 的外层 `await` 只有 512 次、按字节的循环留在 `AsyncLineSequence` 内部同一执行器上；改到调用点逐字节 `await` 后，537k 个字节每个都要跳出 MainActor 再跳回来。修法：新增 `SSEEventReader.read`，用 `@concurrent` 把字节读取/`SSEFramer` 组帧/JSON 解码整体移到通用执行器，只在**完整事件**的交付边界回调 MainActor 做 `checkCancellation` + `validate(lease)` + `yield`，切服与取消语义不变。**`@concurrent` 是承重的**：工程开了 `SWIFT_APPROACHABLE_CONCURRENCY`（含 NonisolatedNonsendingByDefault），只写 `nonisolated` 仍继承调用者执行器 —— 实测仅删掉 `@concurrent`、其余逐字不动，回归原样复现（11.377 s）。②[P2] 三轮收紧成 `guard subscribe != nil` 仍是半截：`subscribe_id` 是**分享方实例的本地订阅自增编号**，跨实例必然重复（各实例都从 1 发号）。分享 `id` 与 `share_uid` 都缺、两实例同号、同媒体同季，且分享人缺失或显示名相同时，两条不同分享被并成一条。**修前先回后端源码坐实前提**（这一步前几轮做得不够）：`share_uid` = `MoviePilotServerHelper.get_user_uuid()`（`helper/server.py:110-118`）→ `SystemUtils.generate_user_unique_id()`（`utils/system.py:933-973`，根文件系统 `st_dev-st_ino` 的 SHA-256，回退 MAC），`server.py:52-58` 自述「当前安装**实例**…稳定用户 ID」—— 是安装实例 ID，不是账号 ID，也不随显示名变；`share_user` 是前端自由填写的显示名，后端不校验。故 (实例, 本地订阅号) 足以确定一条分享。修法据此收紧为**同时**要求 `share_uid` 与正数 `subscribe_id`，并把 `share_user` 整体移出身份构造。**为何不照 Web**：Web 的 `get-item-key` 正是 `e.id \|\| 媒体标识链-share_user`（用可改名的显示名），且列表侧裸 `concat` 不去重；TV 的 `hasMore` 收敛依赖去重，不能照抄，而 Web key 里 `share_user` 那个位置恰是本项两轮反例的来源。 | 8 | 新增 8 条：P1 的 `SSEStreamTests` 4 条（1 条吞吐判别 + 3 条阴性对照：多行/BOM/无终止尾组帧、消费者取消停流、切服停旧流）；P2 的 3 条反例 + 1 条代价固化（缺 `share_uid` 时跨页不再合并）。另把两条在新实现下对身份**已无判别力**的用例按本仓口径降级标注为阴性对照并写明理由，不留在原位冒充回归证据。**反向验证逐项做、每次只退一个文件到 HEAD**：SSE → 吞吐用例 `production=11.089317 s` 转挂、其余 3 条全过（阴性对照成立）；Models → 3 条反例（`("1") is not equal to ("2")`）+ 1 条无 UID 代价用例转挂；再单删 `@concurrent` 复现 11.377 s，确认该标注承重。修复后同一条用例 `production=0.331 s`（baseline 0.034 s）。全量 **1039/1039 通过、零失败**（1029 + 10）。 **残留照实记**：字节级组帧仍有约 9 倍于 `.lines` 的固有开销（需改 `URLSessionDataDelegate` 分块读取才能消除，本轮不做）；吞吐用例是墙钟断言，对 1 s 地板 3 倍余量，机器重载有抖动风险；`share_uid` 不保证全局唯一（同 rootfs 的实例会得到相同值，这类实例在同样三元组下仍会并卡 —— 比修复前窄得多但没归零）、生成失败时为空串（等同缺失→随机身份，安全方向）、且**不持久化**（容器重建即换值，后端自己的 Follow 也会静默失配）。 |
| S482 | F-118/F-138 当前处置校准登记 | 独立子代理审查当前 `ai/p3-audit-dispositions` 相对 `main` 的 10 个提交后确认：F-118 的历史 ownerless pin 根因已被后续内存生命周期实现覆盖，`1a30c01` 引入多owner `navigationOwners`，`9f55542` 以 `isTaskRetained` 补强共享任务保护，`84d910a` 再以稳定 `ImageNavigationEntry.id` 把Push acquire与路径终态release接入当前typed-route生命周期；F-138 同一摘要行已记录 `ff4ea14` 修复和验证，却仍误标“已确认”。本次只同步四份审计文档，将 F-118/F-138 当前状态改为已修复，不改写历史 S 批次裁决时间线，不修改生产代码或测试。 | 0 | manager层已有 `testPopReleasesTaskOnlyAfterLastNavigationOwnerLeaves`，生产接线层已有 `testSameMediaUsesIndependentRouteOwnersAcrossStacks`；本轮独立审查覆盖10个提交/23个文件，`git diff --check main...HEAD`通过，前一轮定向测试275/275、TMDB防重入1/1通过。本轮文档复核未重跑XCTest；未跑完整套件、真实后端兼容、真机push/Tab/focus/VoiceOver或内存验收。 |
| S483 | F-172 通用缺图图标修复登记 | 用户在确认“占位”指海报区域中央而非左上角徽章后，选择未知类型使用通用系统`photo`。实现新增共享`MediaTypePresentation`：电影/电视剧/合集/人物保留专用占位图标，nil、空值、订阅状态文字及未知类型统一`photo`；BadgeOverlay维持原有三类型图标/其他值文字边界，不改变左上徽章；BestResultCard删除重复switch；DetailCard、Search最佳结果和ManualMediaSearch入口改传`displayTypeText`，使collection_id、`collection`与`系列`统一显示合集图标。 | 4 | `MediaInfoCollectionBehaviorTests`新增4条，覆盖4种已知占位类型、7种未知/状态值、左上徽章既有边界和3种合集形态；定向28/28通过，依赖解析和tvOS Simulator clean build通过。标准串行测试在`BackendCompatibilityReadOnlyTests.testReadOnlyTVSurfaceCompatibilityAndImageRendering`因真实后端`/mediaserver/latest`返回502失败（10项中1项失败）后按规则停止，未隔离重跑、调查后端或继续副作用套件；真机缺图视觉未验收。 |
| S484 | F-117 后续内存优化复核与最终跳过登记 | 重新按当前分支复核图片预取取消链：后续内存优化已增加请求前取消检查、`MPImageWarmer`取得handle后的取消、continuation exactly-once与晚到内存结果代际丢弃；但直接Kingfisher路径仍把取消状态与`activeImageDownload`分别保存，`onCancel`可在handle安装前看到nil，随后请求仍被创建且无人二次取消；`loadDetail`在预取返回后也仍无取消复查便写`isDetailReady`。故F-117并未被后续优化完全修复，只是残余影响收窄。 | 0 | 未修改生产代码或测试。现有`MPImageWarmerTests.testCancelledPreloadRemovesLateBackgroundResultAfterCompletion`只覆盖晚到结果丢弃，不覆盖handle安装竞态。用户认为P3窗口极小且改动取消/句柄交接更易引入问题，决定于2026-09-17保持现状并标记跳过。 |
| S485 | F-119 当前实现、配套Web复核与最终跳过登记 | 当前TV仍按`MediaInfo.id`缓存任务，同一`apiMediaId`的富/简对象可形成多个任务；内存生命周期收紧只减少失去owner的普通alias，导航owner、焦点候选与附带预载仍可并存。`findTask(byMediaId:)`仍以`first(where:)`只返回一个，保存/取消的直接回写未覆盖全部canonical任务。目标后端v3.0.1绑定的Web v3.0.1虽以canonical身份+season共享普通Map缓存、没有first-match任务根因，但已挂载卡片各持本地非响应式`isSubscribed`，mutation只改当前实例，其他已完成首次查询的同媒体实例也可能保留旧标签；重复卡片测试只覆盖初查请求合并。 | 0 | 未修改生产代码或测试。TV点击时仍进行权威状态查询，Web/TV均未证明旧标签会导致错误mutation；真实多实例并存频率未验证。用户决定于2026-09-17不做TV单端增强并标记跳过。 |
| S486 | F-238 配套Web与重复query合同复核后跳过登记 | 当前TV的`appendingQuery`会保留插件`api_path`已有查询并追加筛选参数；配套Web v3.0.1同样由`ExtraSourceView`原样传`api_path`、由`MediaCardListView`以Axios params追加`filter_params`，同名时形成相同重复键。现场核对标准Starlette查询：`mode=old&mode=new`下`get("mode")`返回`new`，`getlist("mode")`返回`[old,new]`；因此普通FastAPI标量参数让后追加的当前筛选值生效，列表/显式多值则按多值合同处理。核心与配套Web未提供同名真实插件fixture，特殊插件自定义取首解析仍未知。 | 0 | 未修改生产代码或测试。用户决定保持Web一致，不在TV单独定义同名替换语义，于2026-09-17标记跳过；官方插件若以后明确不同合同再同步。 |
| S487 | F-241 当前实现复核与修复状态校准 | 原I016只能静态推断App Info Sheet关闭时Menu可能同时触发底层scroll/focus处理，但后续提交`05ba3bd`已修改生产链：`SystemSettingsRootBackObserver.isEnabled`在App Info、更新通知、更新日志或退出确认呈现时为false；`handlePress`另以`windowHasNoPresentedContent()`拦截Sheet/alert关闭动画窗口。双层守卫已关闭原静态触发链，普通root Menu行为不变。 | 0 | 本轮仅校准审计文档，不修改生产代码或测试。现有`SystemViewDefaultStyleTests.testSystemViewExitHandlersOnlyRunWhenSettingsTabIsActive`覆盖四类弹层状态门禁与presented-view-controller兜底，本轮定向回归1/1通过。真机Focus Engine最终落点未单独复验。 |
