...
<details>
<summary>F-061 · P2 · 已修复 · 软过滤置尾被结果页二次排序破坏</summary>
...
- 修复：所有站点资源先按 `isFilteredOut` 做全局分区；默认保留后端顺序，显式排序分别作用于正常和软过滤分区。
- 验证：默认顺序及显式大小排序回归通过；依赖解析、tvOS Simulator Debug完整构建和串行全量测试均通过。
...
<details>
<summary>F-067 · P2 · 已确认（用户决定跳过） · 可选剧集组失败阻断整个订阅编辑页</summary>
...
- 最小修改方向 / 裁决：不拆分加载失败域，维持整体失败与重试；用户决定跳过，不做TV单端增强。

</details>

<details>
<summary>F-068 · P2 · 已确认（用户决定跳过） · nil/0/重复业务 ID 可进入订阅快照</summary>
...
- 最小修改方向 / 裁决：不做 TV 单端异常数据兜底；用户决定跳过。

</details>

<details>
<summary>F-070 · P2 · 已修复 · 未知 AI 能力被当作已启用</summary>
...
- 修复 / 验证：改为 `== true`；覆盖 settings 缺失、字段缺失、null、false、true，并通过定向回归与完整工程验证。

</details>

<details>
<summary>F-071 · P2 · 已修复 · 搜索后 owner 与 fetcher 形成强引用环</summary>
...
- 修复 / 验证：在搜索闭包外冻结局部pageSize；释放回归修复前失败、修复后通过，完整构建和串行全量测试通过。
...
<details>
<summary>F-137 · P2 · 已修复（2026-08-18） · 模糊匹配长度罚分让不匹配项反超真实匹配</summary>
...
- 当前处置：四类带宽互不重叠（全等1000/前缀700/包含400/顺序100-299）且长度罚分封顶；顺序匹配 fzf 风格词首/连续加分；另按用户裁决新增热度加权：订阅分享固定 0.6 参与加权，候选池出现多来源（订阅分享不计）时全部不计算热度；SearchViewModelTests 28/28 通过。
...
<details>
<summary>F-145 · P2 · 已修复（2026-08-18） · 下载器选择无法恢复初始省略状态</summary>
...
- 最小修改方向 / 裁决：复用现有机制做局部收敛；具体边界以发现台账为准。
- 修复状态：已完成（2026-08-18）。下载器 options 前置“自动”空项（与保存路径同款），选中后可在同一 Sheet 改回自动，请求体恢复省略 `downloader`。
- 同类排查：全仓 SheetPicker 使用点均已核对，其余表单（SubscribeSheet/ReorganizeSheet）已有空项，无同类问题。
- 验证：新增 AddDownloadRequestOmissionTests 定向回归（初始省略→选中携带→选回自动恢复省略），共 5 项 0 失败。
...
<details>
<summary>F-165 · P3 · 用户降级（2026-08-20） · 部分 Sheet 缺少明显的内容内退出方式</summary>
...
<details>
<summary>F-168 · P2 · 用户跳过（2026-08-20） · SheetPicker 未把当前选择交给 focus/accessibility</summary>
...
<details>
<summary>F-170 · P2 · 已修复（2026-08-20） · 选项域变化后隐藏的多选值无法移除</summary>
...
<details>
<summary>F-179 · P2 · 已修复（2026-08-20） · 资源卡展示字符串未统一规范空白</summary>
...
<details>
<summary>F-187 · P2 · 用户跳过（2026-08-20） · 资源错误或成功空终态没有同页面重试</summary>
...
<details>
<summary>F-194 · P2 · 用户跳过（2026-08-20） · Fork 确认页隐藏立即持久化的关键搜索规则</summary>
...
<details>
<summary>F-195 · P2 · 用户跳过（2026-08-20） · `custom_words` 多规则合同被降成单行编辑</summary>
...
<details>
<summary>F-198 · P2 · 已修复（2026-08-21） · 不可获取的剧集统计被显示为 0</summary>
...
<details>
<summary>F-200 · P2 · 用户决定跳过（2026-08-28） · 保存路径开放合同被封闭 Picker 限制</summary>
...
- 处置状态：用户决定直接跳过（2026-08-28），不修改；保留问题，现状保持不变。
...
<details>
<summary>F-205 · P2 · 用户决定暂缓（2026-08-28） · Reorganize 刷新期间唯一焦点恢复调用被丢弃</summary>
...
- 处置状态：用户决定暂时跳过（2026-08-28），保留问题与后续修复可能；未改代码。

</details>

<details>
<summary>F-206 · P2 · 用户决定跳过（2026-08-28） · Reorganize 无法输入后端支持的自定义目标路径</summary>
...
<details>
<summary>F-209 · P2 · 已修复（2026-08-28） · “全部站点”被编码成后端默认站点子集</summary>
...
- 修复状态：已完成（2026-08-28）；`SiteFilterViewModel.sitesString` 空选择显式发送全部启用站点 ID（过滤 `is_active`），启用站点为空降级 nil，具体选择不变；domain 修复（A 方案）：TV 站点来源切换权威 `/site/` 域——`APIService.fetchAllSites`/`fetchSearchableSites`（有 manage 优先权威域、失败或无 manage 降级 `/site/rss`，返回 authoritative 标志），`SiteFilterViewModel`/`SystemViewModel` 仅 authoritative 时归一化（降级不裁剪已保存合法站点），`SubscribeSheetViewModel` 保持 `/site/rss`；`SiteFilterViewModelSitesStringTests` 增至 10 条，真实后端兼容扩展权威域断言 1/1 通过，受影响类全过。

</details>

<details>
<summary>F-210 · P2 · 已修复（2026-08-28） · 资源搜索站点选择器使用了错误的 RSS 权威域</summary>
...
- 修复状态：已修复（2026-08-28，部分覆盖）；A 方案将 TV 站点来源切换权威 `/site/` 域并滤 inactive、仅权威成功后归一化，覆盖主要破坏面；残留：search-only 无 manage 账号降级订阅域、search 权限可读接口未落地。
...
<details>
<summary>F-242 · P3 · 用户跳过（2026-09-16） · System 动态长名称没有完整可辨识入口</summary>
...
- 最小修改方向 / 裁决：聚焦动态项时在现有preview首行显示完整名称，或允许标题两行；不建长文本组件。用户决定跳过TV端处理（2026-09-16），现状保持不变。
...
<details>
<summary>F-183 · P3 · 已修复（2026-09-16） · TMDB 按钮缺少同步重入边界</summary>

- 审查单元与位置：W008-C；TMDB按钮动作重入
- 触发路径：用户在媒体详情页按“TMDB详情页”时，如果识别还未结束又快速按第二次 Select；当前每次激活都会新建一份异步任务。
- 根因：动作入口此前没有同步的本地进行中标记；两次任务可以同时识别同一媒体，先结束的任务还会把共享的“正在识别媒体信息...”状态提前清掉。
- 用户影响：可能产生重复识别请求，遮罩在另一份识别仍进行时消失；两次都成功时，可能重复尝试进入同一个 TMDB 详情页。现有当前页面校验可拦截部分晚到导航，但不能替代动作级防重入。
- 证据：review_a001_j提出静态链；verify_a001_h不读审计文档第三裁决确认机制成立但tvOS第二次Select可达性无证据。本次在按钮动作入口同步设置本地进行中标记，`TMDBJumpReentrancyCallSiteTests.testDetailJumpKeepsPreloadDisableAndGuardsReentrancy`定向测试1/1通过；临时去掉防重入守卫时反向校验按预期失败，恢复后通过；tvOS Simulator clean build通过。
- 跨端结论：已按TV现有动作链修复；预识别期间继续保留原有disabled门禁，不新增任务期间的disabled状态，因此没有扩大焦点变化范围。本次未跑兼容测试，真实第二次Select可达性、遮罩投递和重复导航表现仍未单独运行验证。
- 最小修改方向 / 裁决：已修复。保留预识别期间的原有disabled，第一次可执行动作开始前设置本地in-flight；任务成功、失败或取消都清除标记，重复激活直接返回；保留既有当前页面校验，不改识别接口或导航框架。
